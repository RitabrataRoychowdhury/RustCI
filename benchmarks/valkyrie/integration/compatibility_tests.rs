...
mod compatibility_tests {
    ...
    async fn test_kubernetes_service_discovery() {
        ...
        let connection_result = timeout(Duration::from_secs(5), engine.connect(bind_addr)).await;

        assert!(
            connection_result.is_ok(),
            "Kubernetes service connection should work"
        );
    ...
    async fn test_kubernetes_pod_to_pod_communication() {
        ...
        let connection = timeout(Duration::from_secs(5), pod1.connect(pod2_addr))
            .await
            .unwrap()
            .unwrap();
    ...
    async fn test_docker_container_networking() {
        ...
        let connection = timeout(Duration::from_secs(5), engine.connect(bind_addr))
            .await
            .unwrap()
            .unwrap();
        ...
        assert!(
            send_result.is_ok(),
            "Docker container communication should work"
        );
    ...
    async fn test_docker_compose_service_communication() {
        ...
        let connection = timeout(Duration::from_secs(5), api_service.connect(worker_addr))
            .await
            .unwrap()
            .unwrap();
        ...
        assert!(
            send_result.is_ok(),
            "Docker Compose service communication should work"
        );
    ...
    async fn test_kubernetes_configmap_integration() {
        ...
        config.bind_port = configmap_data
            .get("VALKYRIE_BIND_PORT")
            .unwrap()
            .parse()
            .unwrap();
        config.security_enabled = configmap_data
            .get("VALKYRIE_SECURITY_ENABLED")
            .unwrap()
            .parse()
            .unwrap();
        ...
        assert!(
            start_result.is_ok(),
            "Engine should start with ConfigMap configuration"
        );
    ...
    async fn test_kubernetes_secret_integration() {
        ...
        std::fs::write(
            &cert_path,
            base64::decode(secret_data.get("tls.crt").unwrap()).unwrap(),
        )
        .unwrap();
        std::fs::write(
            &key_path,
            base64::decode(secret_data.get("tls.key").unwrap()).unwrap(),
        )
        .unwrap();
        ...
        config.auth_token = Some(
            String::from_utf8(base64::decode(secret_data.get("auth.token").unwrap()).unwrap())
                .unwrap(),
        );
        ...
        assert!(
            start_result.is_ok(),
            "Engine should start with Kubernetes secrets"
        );
    ...
    async fn test_docker_volume_persistence() {
        ...
        // First engine instance
        {
            ...
            engine
                .save_persistent_data("test-key", "test-value")
                .await
                .unwrap();
    ...
    async fn test_docker_network_modes() {
        ...
        for (mode, network_cidr) in network_modes {
            ...
            assert!(
                start_result.is_ok(),
                "Engine should work with {} network mode",
                mode
            );
            ...
            assert!(
                connection.is_ok(),
                "Network connectivity should work in {} mode",
                mode
            );
    ...
    async fn test_kubernetes_horizontal_pod_autoscaler() {
        ...
        assert_eq!(
            total_replicas, max_replicas,
            "Should have scaled to max replicas"
        );
    ...
    async fn test_docker_swarm_service_discovery() {
        ...
        for (service_name, endpoints) in swarm_services {
            ...
            for endpoint in endpoints {
                ...
                let connection = timeout(Duration::from_secs(5), engine.connect(bind_addr)).await;

                assert!(
                    connection.is_ok(),
                    "Swarm service endpoint should be reachable"
                );
