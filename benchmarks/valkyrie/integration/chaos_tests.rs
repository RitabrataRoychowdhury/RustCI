...
use crate::valkyrie::{ChaosScenario, TestAssertions, TestFixtures, TestUtils};
...
mod chaos_tests {
    ...
    async fn test_network_partition_recovery() {
        ...
        engines[0]
            .simulate_network_partition(vec![1, 2])
            .await
            .unwrap();
        ...
        assert!(
            partition_connection.is_ok(),
            "Non-partitioned nodes should still communicate"
        );
        ...
        let recovery_connection = timeout(Duration::from_secs(10), engines[0].connect(addr)).await;

        assert!(
            recovery_connection.is_ok(),
            "Nodes should reconnect after partition healing"
        );
    ...
    async fn test_high_latency_injection() {
        ...
        for _ in 0..10 {
            ...
            let (_, latency) =
                TestUtils::measure_time(async { connection.send_message(message).await }).await;
        ...
        engine
            .inject_network_latency(Duration::from_millis(500))
            .await
            .unwrap();
        ...
        let high_latency_avg = high_latency_measurements.iter().sum::<Duration>()
            / high_latency_measurements.len() as u32;
    ...
    async fn test_packet_loss_simulation() {
        ...
        println!(
            "Packet loss success rate: {:.2}%",
            packet_loss_success_rate * 100.0
        );
    ...
    async fn test_node_failure_and_recovery() {
        ...
        assert!(
            failed_send.is_err(),
            "Connection to failed node should fail"
        );
        ...
        assert!(
            working_send.is_ok(),
            "Connection to healthy node should work"
        );
        ...
        let recovered_connection =
            timeout(Duration::from_secs(10), engines[0].connect(addr1)).await;

        assert!(
            recovered_connection.is_ok(),
            "Should reconnect to recovered node"
        );
    ...
    async fn test_memory_pressure_simulation() {
        ...
        let (_, baseline_latency) =
            TestUtils::measure_time(async { connection.send_message(baseline_message).await })
                .await;
        ...
        let (_, pressure_latency) =
            TestUtils::measure_time(async { connection.send_message(pressure_message).await })
                .await;
        ...
        let (_, recovery_latency) =
            TestUtils::measure_time(async { connection.send_message(recovery_message).await })
                .await;
    ...
    async fn test_cpu_starvation_simulation() {
        ...
        println!(
            ...
            baseline_throughput.ops_per_second
        );
        ...
        println!(
            "CPU starvation throughput: {:.2} ops/sec",
            starvation_throughput.ops_per_second
        );
        ...
        println!(
            "Recovery throughput: {:.2} ops/sec",
            recovery_throughput.ops_per_second
        );
    ...
    async fn test_cascading_failure_scenario() {
        ...
        let surviving_connection = timeout(Duration::from_secs(5), engines[2].connect(addr3)).await;

        assert!(
            surviving_connection.is_ok(),
        ...
        assert!(
            send_result.is_ok(),
            "Surviving nodes should handle messages"
        );
        ...
        let recovery_connection = timeout(Duration::from_secs(10), engines[2].connect(addr0)).await;

        assert!(
            recovery_connection.is_ok(),
            "All nodes should communicate after recovery"
        );
    ...
    async fn test_chaos_scenarios_from_fixtures() {
        ...
        for scenario in scenarios {
            ...
            match scenario.name.as_str() {
                ...
                "high_latency" => {
                    engine
                        .inject_network_latency(Duration::from_millis(100))
                        .await
                        .unwrap();
                }
                "packet_loss" => {
            ...
            assert!(
                ...
                "System should recover from chaos scenario: {}",
                scenario.name
            );
