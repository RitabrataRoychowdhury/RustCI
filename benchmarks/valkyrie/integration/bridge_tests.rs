...
mod bridge_tests {
    ...
    async fn test_websocket_upgrade() {
        ...
        assert!(
            ws_stream.get_ref().peer_addr().is_ok(),
    ...
    async fn test_bridge_performance() {
        ...
        println!(
            "  Sub-millisecond HTTP bridge requests: {}/{} ({:.2}%)",
    ...
    async fn test_dns_load_balancer_integration() {
        ...
        for i in 0..request_count {
            ...
            let response = client
                .post(&url)
                .json(&json!({ "pipeline": format!("lb-test-{}", i) }))
                .send()
                .await;
    ...
    async fn test_kubernetes_ingress_compatibility() {
        ...
        assert!(
            response.status().is_success(),
            "Kubernetes ingress request should succeed"
        );
    ...
    async fn test_docker_api_compatibility() {
        ...
        assert!(
            response.status().is_success(),
            "Docker API request should succeed"
        );
    ...
    async fn test_mcp_like_functionality() {
        ...
        assert!(
            response.status().is_success(),
            "MCP tools discovery should succeed"
        );

        let tools: serde_json::Value = response.json().await.unwrap();
        assert!(
            tools.get("tools").is_some(),
