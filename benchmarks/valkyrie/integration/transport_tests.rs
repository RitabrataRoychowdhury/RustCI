...
use rustci::core::networking::valkyrie::transport::{Transport, TransportType};
...
mod transport_tests {
    ...
    async fn test_quic_transport() {
        ...
        assert!(
            connection.is_connected(),
            "QUIC connection should be established"
        );
    ...
    async fn test_unix_socket_transport() {
        #[cfg(unix)]
        {
            ...
            assert!(
                connection.is_connected(),
                "Unix socket connection should be established"
            );
    ...
    async fn test_transport_failover() {
        let mut config = TestFixtures::minimal_config();
        config.fallback_transports = vec![TransportType::Tcp, TransportType::WebSocket];
    ...
    async fn test_transport_message_sizes() {
        ...
        for size in message_sizes {
            ...
            let (_, send_time) =
                TestUtils::measure_time(async { engine.send_message(message).await }).await;
    ...
    async fn test_concurrent_connections() {
        ...
        for _ in 0..connection_count {
            ...
            let task = tokio::spawn(async move { engine_clone.connect(bind_addr).await });
    ...
    async fn test_transport_multiplexing() {
        ...
        for i in 0..message_count {
            ...
            let task = tokio::spawn(async move { connection_clone.send_message(message).await });
        ...
        assert_eq!(
            ...
            "All multiplexed messages should be sent"
        );
