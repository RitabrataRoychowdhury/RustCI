...
use rustci::core::networking::valkyrie::security::{AuthMethod, EncryptionMethod, SecurityManager};
...
mod security_tests {
    ...
    async fn test_mutual_tls_authentication() {
        ...
        assert!(
            ...
            "mTLS connection should be established"
        );
    ...
    async fn test_token_based_authentication() {
        ...
        let valid_connection = engine
            .connect_with_token("127.0.0.1:8080".parse().unwrap(), "test-secret-token-123")
            .await;

        assert!(valid_connection.is_ok(), "Valid token should authenticate");

        // Test invalid token authentication
        let invalid_connection = engine
            .connect_with_token("127.0.0.1:8080".parse().unwrap(), "invalid-token")
            .await;

        assert!(
            invalid_connection.is_err(),
    ...
    async fn test_post_quantum_cryptography() {
        ...
        assert_eq!(
            encryption_info.key_exchange, "kyber",
            "Should use Kyber key exchange"
        );
        assert_eq!(
            encryption_info.signature, "dilithium",
            "Should use Dilithium signatures"
        );
    ...
    async fn test_intrusion_detection_system() {
        ...
        assert!(
            !ids_alerts.is_empty(),
            "IDS should detect suspicious activity"
        );

        let alert = &ids_alerts[0];
        assert_eq!(
            alert.alert_type, "rapid_connections",
            "Should detect rapid connections"
        );
    ...
    async fn test_rbac_authorization() {
        ...
        engine
            .create_role(
                "admin",
                vec!["job:create", "job:read", "job:update", "job:delete"],
            )
            .await
            .unwrap();
        ...
        let admin_token = engine
            .create_user_token("admin-user", "admin")
            .await
            .unwrap();
        let admin_authorized = engine
            .check_permission(&admin_token, "job:create")
            .await
            .unwrap();
        assert!(admin_authorized, "Admin should have create permission");

        // Test user permissions
        let user_token = engine
            .create_user_token("regular-user", "user")
            .await
            .unwrap();
        let user_create_denied = engine
            .check_permission(&user_token, "job:create")
            .await
            .unwrap();
        assert!(
            !user_create_denied,
        ...
        let user_read_allowed = engine
            .check_permission(&user_token, "job:read")
            .await
            .unwrap();
    ...
    async fn test_audit_logging() {
        ...
        engine
            .send_message_via_connection(&connection, message)
            .await
            .unwrap();
        ...
        assert!(
            audit_log.contains("CONNECTION_ESTABLISHED"),
            "Should log connection"
        );
        assert!(
            audit_log.contains("MESSAGE_SENT"),
            "Should log message sending"
        );
        ...
        for entry in audit_entries {
            ...
            assert!(
                ...
                "Audit entry should have event type"
            );
            assert!(
                entry.get("integrity_hash").is_some(),
                "Audit entry should have integrity hash"
            );
        }
    ...
    async fn test_certificate_rotation() {
        ...
        assert!(
            connection.is_ok(),
            "Connection should work with rotated certificate"
        );
    ...
    async fn test_zero_trust_architecture() {
        ...
        assert!(
            unauthorized_connection.is_err(),
            "Unauthorized connection should be denied"
        );

        // Add explicit allow policy
        engine
            .add_security_policy("allow", "127.0.0.1", vec!["connect", "send_message"])
            .await
            .unwrap();
        ...
        assert!(
            authorized_connection.is_ok(),
            "Authorized connection should succeed"
        );
    ...
    async fn test_encryption_performance() {
        ...
        for (method, is_pq) in encryption_methods {
            ...
            let (_, encryption_time) =
                TestUtils::measure_time(async { engine.encrypt_data(&test_data).await }).await;
