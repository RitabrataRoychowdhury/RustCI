...
use crate::valkyrie::{LoadScenario, PerformanceUtils, TestAssertions, TestFixtures, TestUtils};
...
mod performance_tests {
    ...
    async fn test_ultra_low_latency() {
        ...
        println!(
            "  Min: {:?} ({:.0}ns)",
            ultra_stats.min,
            ultra_stats.min.as_nanos() as f64
        );
        println!(
            "  P50: {:?} ({:.0}ns)",
            ultra_stats.p50,
            ultra_stats.p50.as_nanos() as f64
        );
        println!(
            "  P95: {:?} ({:.0}ns)",
            ultra_stats.p95,
            ultra_stats.p95.as_nanos() as f64
        );
        println!(
            "  P99: {:?} ({:.0}ns)",
            ultra_stats.p99,
            ultra_stats.p99.as_nanos() as f64
        );
        println!(
            "  Mean: {:?} ({:.0}ns)",
            ultra_stats.mean,
            ultra_stats.mean.as_nanos() as f64
        );
        ...
        let ultra_fast_count = ultra_low_latencies
            .iter()
    ...
    async fn test_micro_benchmark_critical_path() {
        ...
        println!(
            "  Min: {:?} ({:.0}ns)",
            micro_stats.min,
            micro_stats.min.as_nanos() as f64
        );
        println!(
            "  P50: {:?} ({:.0}ns)",
            micro_stats.p50,
            micro_stats.p50.as_nanos() as f64
        );
        println!(
            "  P95: {:?} ({:.0}ns)",
            micro_stats.p95,
            micro_stats.p95.as_nanos() as f64
        );
        println!(
            "  P99: {:?} ({:.0}ns)",
            micro_stats.p99,
            micro_stats.p99.as_nanos() as f64
        );
        println!(
            "  P99.9: {:?} ({:.0}ns)",
            micro_stats.p999,
            micro_stats.p999.as_nanos() as f64
        );
        ...
        let ultra_fast_count = micro_latencies
            .iter()
    ...
    async fn test_high_throughput() {
        ...
        println!(
            "  Avg latency: {:.2}μs",
            1_000_000.0 / throughput_stats.ops_per_second
        );
    ...
    async fn test_memory_usage_scalability() {
        ...
        println!(
            ...
            (peak_memory - initial_memory) / connection_count / 1024
        );
    ...
    async fn test_load_scenarios() {
        ...
        for scenario in scenarios {
            ...
            let messages_per_connection =
                scenario.messages_per_second / scenario.concurrent_connections;
            ...
            let (total_successful, total_failed): (u64, u64) = results
                .into_iter()
    ...
    async fn test_cpu_utilization() {
        ...
        assert!(
            max_cpu < 95.0,
            "Peak CPU usage should not saturate the system"
        );
    ...
    async fn test_network_bandwidth_utilization() {
        ...
        for message_size in message_sizes {
            ...
            assert!(
                bandwidth_mbps > 10.0,
                "Should achieve reasonable bandwidth utilization"
            );
        }
