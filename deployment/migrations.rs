...
impl MigrationRunner for MongoMigrationRunner {
    ...
    async fn get_applied_migrations(&self) -> Result<Vec<Migration>, MigrationError> {
        // In a real implementation, this would query the migrations collection
        // For now, return empty vec
        Ok(Vec::new())
    }

    async fn record_migration(&self, migration: &Migration) -> Result<(), MigrationError> {
        // In a real implementation, this would insert into migrations collection
        println!("📝 Recording migration: {}", migration.id);
        Ok(())
    }

    async fn remove_migration_record(&self, migration_id: &str) -> Result<(), MigrationError> {
        // In a real implementation, this would remove from migrations collection
        println!("🗑️ Removing migration record: {}", migration_id);
        Ok(())
    }
}
...
impl MongoMigrationRunner {
    async fn execute_script(&self, script: &str) -> Result<(), MigrationError> {
        // In a real implementation, this would execute MongoDB operations
...
impl MigrationManager {
    ...
    pub fn create_migration_template(&self, name: &str, description: &str) -> String {
...
use crate::deployment::migrations::{{Migration, MigrationError}};

pub fn create_migration() -> Migration {{
    Migration::new(
        "{timestamp}_{name}",
        "{name}",
        {version},
        "{description}",
        r#"
        // Up script - add your MongoDB operations here
        // Example:
        // db.users.createIndex({{ "email": 1 }}, {{ unique: true }})
        // db.workspaces.updateMany({{}}, {{ $set: {{ "status": "active" }} }})
        "#,
        r#"
        // Down script - add rollback operations here
        // Example:
        // db.users.dropIndex("email_1")
        // db.workspaces.updateMany({{}}, {{ $unset: {{ "status": "" }} }})
        "#,
    )
}}
"#,
    ...
    pub fn print_status(&self) {
        ...
        for migration in &self.applied_migrations {
            println!("   v{}: {} (applied: {})", 
                migration.version, 
                migration.name,
                migration.applied_at.unwrap().format("%Y-%m-%d %H:%M:%S")
            );
        }
...
pub fn create_initial_schema_migration() -> Migration {
    Migration::new(
        ...
        // Create workspaces collection with indexes
        db.workspaces.createIndex({ "name": 1, "owner_id": 1 }, { unique: true })
        db.workspaces.createIndex({ "owner_id": 1 })
        db.workspaces.createIndex({ "status": 1 })
        
        // Create pipelines collection with indexes
        db.pipelines.createIndex({ "name": 1, "workspace_id": 1 }, { unique: true })
        db.pipelines.createIndex({ "workspace_id": 1 })
        
        // Create executions collection with indexes
        db.executions.createIndex({ "pipeline_id": 1 })
        db.executions.createIndex({ "status": 1 })
        db.executions.createIndex({ "started_at": -1 })
        "#,
        r#"
