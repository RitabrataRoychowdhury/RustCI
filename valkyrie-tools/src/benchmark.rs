...
pub async fn run_benchmark(
    ...
    println!("  Requests per connection: {}", requests.to_string().yellow());
    println!("  Message size: {} bytes", message_size.to_string().yellow());
    if duration_secs > 0 {
    ...
    println!("  Throughput: {:.1} req/s", success_count as f64 / total_duration.as_secs_f64());
    println!();
    ...
    if success_count > 0 {
        ...
        println!("  P50: {}", histogram.value_at_quantile(0.5).to_string().yellow());
        println!("  P95: {}", histogram.value_at_quantile(0.95).to_string().yellow());
        println!("  P99: {}", histogram.value_at_quantile(0.99).to_string().yellow());
        println!("  P99.9: {}", histogram.value_at_quantile(0.999).to_string().yellow());
    }
    ...
    if error_count == 0 {
        ...
        println!("{} Benchmark completed with {} errors", "⚠".yellow().bold(), error_count);
    }
