...
use crate::core::networking::valkyrie::engine::{
    AuthMethod, CipherSuite, ConnectionReuseStrategy, LoadBalancingStrategy, LogFormat,
    TransportSelectionStrategy, ValkyrieConfig,
};
...
impl ValkyrieConfigManager {
    ...
    pub fn load_from_env(&mut self) -> Result<()> {
        ...
        if let Ok(compat_mode) = std::env::var("VALKYRIE_COMPATIBILITY_MODE") {
            config.protocol.compatibility_mode = compat_mode
                .parse()
        ...
        if let Ok(bridge_enabled) = std::env::var("VALKYRIE_BRIDGE_ENABLED") {
            let _enabled: bool = bridge_enabled.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid bridge enabled setting: {}", e))
            })?;
            // Store in config
        }
        ...
        if let Ok(http_port) = std::env::var("VALKYRIE_HTTP_PORT") {
            ...
            if port < 1024 && port != 80 {
                return Err(AppError::ConfigError(
                    "HTTP port should be >= 1024 or 80".to_string(),
                ));
            }
            // Store in config
        }
        ...
        if let Ok(https_port) = std::env::var("VALKYRIE_HTTPS_PORT") {
            ...
            if port < 1024 && port != 443 {
                return Err(AppError::ConfigError(
                    "HTTPS port should be >= 1024 or 443".to_string(),
                ));
            }
            // Store in config
        }
            ...
            if worker_threads != "auto" {
                let threads: usize = worker_threads.parse().map_err(|e| {
                    AppError::ConfigError(format!(
                        "Invalid worker threads (use 'auto' or number): {}",
                        e
                    ))
                })?;
                if threads == 0 || threads > 1024 {
                    return Err(AppError::ConfigError(
                        "Worker threads must be between 1 and 1024".to_string(),
                    ));
                }
        ...
        if let Ok(node_id) = std::env::var("VALKYRIE_NODE_ID") {
            if node_id.is_empty() || node_id.len() > 64 {
                return Err(AppError::ConfigError(
                    "Node ID must be between 1 and 64 characters".to_string(),
                ));
            }
            // Store in config
        }
        ...
        if let Ok(heartbeat_interval) = std::env::var("VALKYRIE_HEARTBEAT_INTERVAL_SECS") {
            let secs: u64 = heartbeat_interval
                .parse()
        ...
        if let Ok(reuse_strategy) = std::env::var("VALKYRIE_CONNECTION_REUSE_STRATEGY") {
            config.transport.connection_pooling.reuse_strategy =
                match reuse_strategy.to_lowercase().as_str() {
                    ...
                    _ => {
                        return Err(AppError::ConfigError(format!(
                            "Invalid connection reuse strategy: {}",
                            reuse_strategy
                        )))
                    }
                };
        ...
        if let Ok(mutual_auth) = std::env::var("VALKYRIE_REQUIRE_MUTUAL_AUTH") {
            config.security.authentication.require_mutual_auth =
                mutual_auth.parse().map_err(|e| {
                    AppError::ConfigError(format!("Invalid mutual auth setting: {}", e))
                })?;
        }
        ...
        if let Ok(key_rotation) = std::env::var("VALKYRIE_KEY_ROTATION_INTERVAL_SECS") {
            let secs: u64 = key_rotation.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid key rotation interval: {}", e))
            })?;
        ...
        if let Ok(forward_secrecy) = std::env::var("VALKYRIE_FORWARD_SECRECY") {
            config.security.encryption.forward_secrecy = forward_secrecy.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid forward secrecy setting: {}", e))
            })?;
        }

        // Streaming configuration
        if let Ok(max_streams) = std::env::var("VALKYRIE_MAX_STREAMS_PER_CONNECTION") {
            let max: u32 = max_streams
                .parse()
        ...
        if let Ok(initial_window) = std::env::var("VALKYRIE_INITIAL_WINDOW_SIZE") {
            let size: u32 = initial_window.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid initial window size: {}", e))
            })?;
            config.streaming.flow_control.initial_window_size = size;
        }

        if let Ok(max_window) = std::env::var("VALKYRIE_MAX_WINDOW_SIZE") {
            let size: u32 = max_window
                .parse()
        ...
        if let Ok(priority_scheduling) = std::env::var("VALKYRIE_PRIORITY_SCHEDULING") {
            config.streaming.priority_scheduling = priority_scheduling.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid priority scheduling setting: {}", e))
            })?;
        }
        ...
        if let Ok(routing_table_size) = std::env::var("VALKYRIE_ROUTING_TABLE_SIZE") {
            let size: usize = routing_table_size
                .parse()
        ...
        if let Ok(worker_threads) = std::env::var("VALKYRIE_WORKER_THREADS") {
            let threads: usize = worker_threads
                .parse()
        ...
        if let Ok(metrics_enabled) = std::env::var("VALKYRIE_METRICS_ENABLED") {
            config.observability.metrics.enabled = metrics_enabled.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid metrics enabled setting: {}", e))
            })?;
        }

        if let Ok(export_interval) = std::env::var("VALKYRIE_METRICS_EXPORT_INTERVAL_SECS") {
            let secs: u64 = export_interval.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid metrics export interval: {}", e))
            })?;
        ...
        if let Ok(tracing_enabled) = std::env::var("VALKYRIE_TRACING_ENABLED") {
            config.observability.tracing.enabled = tracing_enabled.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid tracing enabled setting: {}", e))
            })?;
        }

        if let Ok(sampling_rate) = std::env::var("VALKYRIE_TRACING_SAMPLING_RATE") {
            let rate: f64 = sampling_rate
                .parse()
        ...
        if let Ok(structured_logging) = std::env::var("VALKYRIE_STRUCTURED_LOGGING") {
            config.observability.logging.structured = structured_logging.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid structured logging setting: {}", e))
            })?;
        }

        // Feature flags
        if let Ok(experimental) = std::env::var("VALKYRIE_EXPERIMENTAL_FEATURES") {
            config.features.experimental = experimental.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid experimental features setting: {}", e))
            })?;
        }

        if let Ok(post_quantum) = std::env::var("VALKYRIE_POST_QUANTUM_CRYPTO") {
            config.features.post_quantum_crypto = post_quantum.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid post-quantum crypto setting: {}", e))
            })?;
        }

        if let Ok(ml_optimizations) = std::env::var("VALKYRIE_ML_OPTIMIZATIONS") {
            config.features.ml_optimizations = ml_optimizations.parse().map_err(|e| {
                AppError::ConfigError(format!("Invalid ML optimizations setting: {}", e))
            })?;
        }
        ...
        if let Ok(custom_flags) = std::env::var("VALKYRIE_CUSTOM_FEATURES") {
            for flag in custom_flags.split(',') {
                if let Some((key, value)) = flag.split_once('=') {
                    let bool_value = value.trim().parse().map_err(|e| {
                        AppError::ConfigError(format!(
                            "Invalid custom feature flag value for {}: {}",
                            key, e
                        ))
                    })?;
                    config
                        .features
                        .custom
                        .insert(key.trim().to_string(), bool_value);
                }
            }
        }
    ...
    pub async fn load_from_file(&mut self, path: &str) -> Result<()> {
        let content = tokio::fs::read_to_string(path).await.map_err(|e| {
            AppError::ConfigError(format!(
                "Failed to read Valkyrie config file {}: {}",
                path, e
            ))
        })?;
    ...
    pub fn validate_config(&self, config: &ValkyrieConfig) -> Result<()> {
        ...
        {
            return Err(AppError::ConfigError(
                "Max connections per endpoint must be greater than 0".to_string(),
            ));
        }
        ...
        if config.streaming.max_streams_per_connection == 0 {
            return Err(AppError::ConfigError(
                "Max streams per connection must be greater than 0".to_string(),
            ));
        }
        if config.streaming.buffer_size == 0 {
            return Err(AppError::ConfigError(
                "Stream buffer size must be greater than 0".to_string(),
            ));
        }
        if config.streaming.flow_control.initial_window_size == 0 {
            return Err(AppError::ConfigError(
                "Initial window size must be greater than 0".to_string(),
            ));
        }
        if config.streaming.flow_control.max_window_size
            < config.streaming.flow_control.initial_window_size
        {
            return Err(AppError::ConfigError(
                "Max window size must be greater than or equal to initial window size".to_string(),
            ));
        }
        if config.streaming.flow_control.update_threshold <= 0.0
            || config.streaming.flow_control.update_threshold > 1.0
        {
            return Err(AppError::ConfigError(
                "Update threshold must be between 0.0 and 1.0".to_string(),
            ));
        }
        ...
        if config.routing.routing_table_size == 0 {
            return Err(AppError::ConfigError(
                "Routing table size must be greater than 0".to_string(),
            ));
        }
        ...
        if config.observability.tracing.sampling_rate < 0.0
            || config.observability.tracing.sampling_rate > 1.0
        {
            return Err(AppError::ConfigError(
    ...
    pub fn to_yaml(&self) -> Result<String> {
        serde_yaml::to_string(&self.config).map_err(|e| {
            AppError::ConfigError(format!("Failed to serialize config to YAML: {}", e))
        })
    }
    ...
    pub fn to_json(&self) -> Result<String> {
        serde_json::to_string_pretty(&self.config).map_err(|e| {
            AppError::ConfigError(format!("Failed to serialize config to JSON: {}", e))
        })
    }
...
"#
    ...
    pub fn validate_config_detailed(&self, config: &ValkyrieConfig) -> Result<Vec<String>> {
        ...
        if config
            ...
            && config.security.authentication.methods.len() > 1
        {
            warnings.push(
                "Authentication includes 'None' method alongside others, which may be insecure"
                    .to_string(),
            );
        }
        ...
        {
            warnings.push(
                "Tracing sampling rate is 100%, which may impact performance in production"
                    .to_string(),
            );
        }
        ...
        if config.streaming.max_streams_per_connection > 10000 {
            warnings
                .push("Very high max streams per connection may cause memory issues".to_string());
        }
    ...
    pub fn get_env_vars() -> Vec<(&'static str, &'static str, &'static str)> {
        vec![
            ...
            (
                "VALKYRIE_BIND_ADDRESS",
                "Bind address for Valkyrie protocol",
                "0.0.0.0:8080",
            ),
            (
                "VALKYRIE_MAX_MESSAGE_SIZE",
                "Maximum message size in bytes",
                "67108864",
            ),
            (
                "VALKYRIE_TOKEN_SECRET",
                "Secret key for token authentication",
                "your-secret-key",
            ),
            (
                "VALKYRIE_BRIDGE_ENABLED",
                "Enable HTTP/HTTPS bridge",
                "true",
            ),
            ...
            (
                ...
                "/etc/valkyrie/certs/server.crt",
            ),
            (
                "VALKYRIE_KEY_FILE",
                "TLS private key file path",
                "/etc/valkyrie/certs/server.key",
            ),
            (
                "VALKYRIE_MAX_CONNECTIONS",
                "Maximum concurrent connections",
                "100000",
            ),
            (
                "VALKYRIE_WORKER_THREADS",
                "Number of worker threads",
                "auto",
            ),
            ("VALKYRIE_LOG_LEVEL", "Logging level", "info"),
            (
                "VALKYRIE_METRICS_ENDPOINT",
                "Metrics endpoint path",
                "/metrics",
            ),
            (
                "VALKYRIE_JAEGER_ENDPOINT",
                "Jaeger tracing endpoint",
                "http://localhost:14268/api/traces",
            ),
            ("VALKYRIE_NODE_ID", "Cluster node identifier", "node-1"),
            (
                "VALKYRIE_CLUSTER_NODES",
                "Comma-separated cluster nodes",
                "node1:8080,node2:8080",
            ),
            (
                "VALKYRIE_CONNECT_TIMEOUT_SECS",
                "Connection timeout in seconds",
                "30",
            ),
            (
                "VALKYRIE_SEND_TIMEOUT_SECS",
                "Send timeout in seconds",
                "10",
            ),
            (
                "VALKYRIE_HEARTBEAT_INTERVAL_SECS",
                "Heartbeat interval in seconds",
                "30",
            ),
            (
                "VALKYRIE_IDLE_TIMEOUT_SECS",
                "Idle timeout in seconds",
                "300",
            ),
            (
                "VALKYRIE_TRANSPORT_SELECTION_STRATEGY",
                "Transport selection strategy",
                "primary",
            ),
            (
                "VALKYRIE_MAX_CONNECTIONS_PER_ENDPOINT",
                "Max connections per endpoint",
                "10",
            ),
            (
                "VALKYRIE_CONNECTION_REUSE_STRATEGY",
                "Connection reuse strategy",
                "per_endpoint",
            ),
            (
                "VALKYRIE_AUTH_METHODS",
                "Comma-separated auth methods",
                "token",
            ),
            (
                "VALKYRIE_TOKEN_EXPIRY_SECS",
                "Token expiry in seconds",
                "3600",
            ),
            (
                "VALKYRIE_REQUIRE_MUTUAL_AUTH",
                "Require mutual authentication",
                "false",
            ),
            (
                "VALKYRIE_CIPHER_SUITES",
                "Comma-separated cipher suites",
                "aes256gcm",
            ),
            (
                "VALKYRIE_KEY_ROTATION_INTERVAL_SECS",
                "Key rotation interval",
                "86400",
            ),
            ("VALKYRIE_FORWARD_SECRECY", "Enable forward secrecy", "true"),
            (
                "VALKYRIE_MAX_STREAMS_PER_CONNECTION",
                "Max streams per connection",
                "100",
            ),
            ("VALKYRIE_STREAM_BUFFER_SIZE", "Stream buffer size", "8192"),
            (
                "VALKYRIE_INITIAL_WINDOW_SIZE",
                "Initial flow control window",
                "65536",
            ),
            (
                "VALKYRIE_MAX_WINDOW_SIZE",
                "Maximum flow control window",
                "1048576",
            ),
            (
                "VALKYRIE_PRIORITY_SCHEDULING",
                "Enable priority scheduling",
                "true",
            ),
            (
                "VALKYRIE_LOAD_BALANCING_STRATEGY",
            ...
            (
                "VALKYRIE_METRICS_ENABLED",
                "Enable metrics collection",
                "true",
            ),
            (
                "VALKYRIE_METRICS_EXPORT_INTERVAL_SECS",
                "Metrics export interval",
                "60",
            ),
            (
                "VALKYRIE_TRACING_ENABLED",
                "Enable distributed tracing",
                "true",
            ),
            (
                "VALKYRIE_TRACING_SAMPLING_RATE",
                "Tracing sampling rate",
                "0.1",
            ),
            (
                "VALKYRIE_TRACING_EXPORT_ENDPOINT",
                "Tracing export endpoint",
                "http://localhost:14268/api/traces",
            ),
            ("VALKYRIE_LOG_FORMAT", "Log format", "json"),
            (
                "VALKYRIE_STRUCTURED_LOGGING",
                "Enable structured logging",
                "true",
            ),
            (
                "VALKYRIE_EXPERIMENTAL_FEATURES",
                "Enable experimental features",
                "false",
            ),
            (
                "VALKYRIE_POST_QUANTUM_CRYPTO",
                "Enable post-quantum crypto",
                "false",
            ),
            (
                "VALKYRIE_ML_OPTIMIZATIONS",
                "Enable ML optimizations",
                "false",
            ),
            (
                "VALKYRIE_CUSTOM_FEATURES",
...
mod tests {
    ...
    fn test_json_serialization() {
        ...
        assert_eq!(
            manager.get_config().protocol.version,
            new_manager.get_config().protocol.version
        );
    }
