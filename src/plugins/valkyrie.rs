...
impl ValkyriePlugin {
    ...
    fn extract_valkyrie_config(plugin_config: &PluginConfig) -> Result<ValkyriePluginConfig> {
        if let Some(valkyrie_config) = plugin_config.config.get("valkyrie") {
            serde_json::from_value(valkyrie_config.clone())
                .map_err(|e| AppError::ConfigurationError(format!("Invalid Valkyrie config: {}", e)))
        } else {
...
impl Plugin for ValkyriePlugin {
    ...
    async fn initialize(&mut self, plugin_config: PluginConfig) -> Result<()> {
        ...
        let adapter_config = crate::infrastructure::runners::valkyrie_adapter::ValkyrieAdapterConfig {
            endpoint: valkyrie_config.endpoint.clone(),
            connection_timeout: std::time::Duration::from_millis(valkyrie_config.connection_timeout_ms),
            request_timeout: std::time::Duration::from_millis(valkyrie_config.request_timeout_ms),
            max_connections: valkyrie_config.max_connections,
            enable_pooling: valkyrie_config.enable_pooling,
            enable_metrics: valkyrie_config.enable_metrics,
            enable_http_fallback: valkyrie_config.enable_http_fallback,
        };
        
        match ValkyrieRunnerAdapter::new(adapter_config).await {
            Ok(adapter) => {
                {
    ...
    async fn stats(&self) -> Result<serde_json::Value> {
        ...
        if let Some(adapter) = adapter_guard.as_ref() {
            ...
            let endpoint = {
                let config_guard = self.config.read().await;
                config_guard.as_ref().map(|c| c.endpoint.clone()).unwrap_or_default()
            };
            ...
            let stats = serde_json::json!({
                ...
                "adapter_healthy": adapter.is_healthy().await,
                "endpoint": endpoint,
