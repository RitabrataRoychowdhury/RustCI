...
use super::{
    fallback::{FallbackConfig, FallbackManager},
    health::{HealthMonitorConfig, PluginHealthMonitor},
...
impl PluginIntegrationService {
    ...
    async fn load_valkyrie_plugin(&self) -> Result<()> {
        ...
        // Load the plugin
        self.plugin_manager
            .load_plugin(valkyrie_plugin, plugin_config)
            .await?;

        // Register for health monitoring
        self.health_monitor
            .register_plugin("valkyrie".to_string())
            .await;

        // Register recovery callback
        let plugin_manager = Arc::clone(&self.plugin_manager);
        self.health_monitor
            .register_recovery_callback("valkyrie".to_string(), move |plugin_id| {
