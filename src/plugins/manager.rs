...
use super::{Plugin, PluginConfig, PluginHealth, PluginMetadata, PluginRegistry, PluginState, PluginSystemStats};
...
impl PluginManager {
    ...
    pub async fn start_plugin(&self, plugin_id: &str) -> Result<()> {
        ...
        match tokio::time::timeout(timeout_duration, start_future).await {
            ...
            Err(_) => {
                error!("Plugin {} startup timed out after {}s", plugin_id, self.config.max_startup_time);
                Err(AppError::Timeout(format!("Plugin {} startup timeout", plugin_id)))
            }
        }
    }
            ...
            loop {
                ...
                for health in health_statuses {
                    if !health.healthy && config.auto_restart {
                        warn!("Plugin {} is unhealthy: {}", health.plugin_id, health.message);
                        ...
                        if should_restart {
                            info!("Attempting to restart unhealthy plugin: {}", health.plugin_id);
                            if let Err(e) = registry.start_plugin(&health.plugin_id).await {
                                error!("Failed to restart plugin {}: {}", health.plugin_id, e);
                            }
                        } else {
                            error!("Plugin {} has exceeded maximum restart attempts", health.plugin_id);
                        }
                    }
                }
            }
