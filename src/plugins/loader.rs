...
impl PluginLoader {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        if !self.config.plugin_dir.exists() {
            std::fs::create_dir_all(&self.config.plugin_dir).map_err(|e| {
                AppError::IoError(format!("Failed to create plugin directory: {}", e))
            })?;
            info!(
    ...
    pub async fn is_plugin_modified(&self, plugin_id: &str) -> Result<bool> {
        ...
        if let Some(discovery) = discovered.get(plugin_id) {
            ...
            let current_modified = metadata.modified().map_err(|e| {
                AppError::IoError(format!("Failed to get modification time: {}", e))
            })?;
    ...
    pub fn validate_plugin_compatibility(&self, metadata: &PluginMetadata) -> Result<()> {
        ...
        if metadata.min_rustci_version.as_str() > current_version {
            return Err(AppError::IncompatibleVersion(format!(
                "Plugin {} requires RustCI version {} or higher, current version is {}",
