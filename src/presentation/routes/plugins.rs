...
pub fn create_plugin_routes() -> Router<PluginManagementState> {
    Router::new()
        ...
        // Fallback management
        .route(
            "/plugins/:plugin_id/fallback/activate",
            post(activate_fallback),
        )
        .route(
            "/plugins/:plugin_id/fallback/deactivate",
            post(deactivate_fallback),
        )
        .route(
            "/plugins/:plugin_id/fallback/status",
            get(get_fallback_status),
        )
        // System management
...
async fn get_plugin(
    ...
    let mut data =
        serde_json::to_value(plugin).map_err(|e| AppError::SerializationError(e.to_string()))?;
...
async fn update_plugin_config(
    ...
    state
        .plugin_manager
        .update_plugin_config(&plugin_id, request.config)
        .await?;
...
async fn activate_fallback(
    ...
    state
        .fallback_manager
        .activate_fallback(&plugin_id, strategy)
        .await?;

    Ok(Json(PluginResponse {
        success: true,
        message: format!(
            "Fallback activated for plugin {} with strategy: {}",
            plugin_id, request.strategy
        ),
        data: None,
    }))
