...
use crate::valkyrie::{TransportConfig, Result};
use crate::valkyrie::transport::{Transport, Endpoint, Connection, Listener};
...
impl TransportManager {
    ...
    pub async fn send_message(&self, connection_id: ConnectionId, message: ValkyrieMessage) -> Result<()> {
        let mut connections = self.connections.write().await;
    ...
    async fn initialize_transports(&self) -> Result<()> {
        ...
        if self.config.primary_transport == "tcp" || self.config.fallback_transports.contains(&"tcp".to_string()) {
            // transports.insert("tcp".to_string(), Arc::new(TcpTransport::new()));
        }
        
        // Initialize WebSocket transport
        if self.config.primary_transport == "websocket" || self.config.fallback_transports.contains(&"websocket".to_string()) {
            // transports.insert("websocket".to_string(), Arc::new(WebSocketTransport::new()));
        }
        
        // Initialize Unix socket transport
        if self.config.primary_transport == "unix" || self.config.fallback_transports.contains(&"unix".to_string()) {
    ...
    fn serialize_message(&self, message: &ValkyrieMessage) -> Result<Vec<u8>> {
        ...
        serde_json::to_vec(message)
            .map_err(|e| crate::valkyrie::ValkyrieError::InternalServerError(
