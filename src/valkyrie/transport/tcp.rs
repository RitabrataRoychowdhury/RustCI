...
impl Transport for TcpTransport {
    async fn connect(&self, _endpoint: &Endpoint) -> Result<Box<dyn Connection>> {
        ...
        Err(crate::valkyrie::ValkyrieError::InternalServerError("TCP connect not implemented".to_string()))
    }
    
    async fn listen(&self, _bind_address: std::net::SocketAddr) -> Result<Box<dyn Listener>> {
        // Placeholder implementation
        Err(crate::valkyrie::ValkyrieError::InternalServerError("TCP listen not implemented".to_string()))
    }
