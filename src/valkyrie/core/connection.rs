...
use crate::valkyrie::{ValkyrieEngine, Result};
...
impl ConnectionRegistry {
    ...
    pub async fn cleanup_stale_connections(&self) -> Result<()> {
        ...
        connections.retain(|_, info| {
            now.signed_duration_since(info.last_activity) < stale_timeout
        });
    ...
    pub async fn get_stats(&self) -> ConnectionStats {
        ...
        ConnectionStats {
            total_connections,
            active_connections,
            connecting_connections: connections.values()
                .filter(|info| info.state == ConnectionState::Connecting)
                .count(),
            closing_connections: connections.values()
...
impl Listener {
    ...
    pub fn new(inner: Box<dyn crate::valkyrie::transport::Listener>, engine: Weak<ValkyrieEngine>) -> Self {
        Self { inner, engine }
    }
...
mod tests {
    ...
    async fn test_connection_registry() {
        ...
        registry.register(connection_id, info.clone()).await.unwrap();
