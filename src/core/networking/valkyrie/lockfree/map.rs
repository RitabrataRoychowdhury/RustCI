    ...
    V: Clone + Send + Sync,
{
    ...
    fn find_entry(
        &self,
        bucket_head: *mut MapEntry<K, V>,
        key: &K,
    ) -> (*mut MapEntry<K, V>, *mut MapEntry<K, V>) {
    ...
    V: Clone + Send + Sync,
{
    fn insert(&self, key: K, value: V) -> Result<Option<V>, MapError> {
        ...
        loop {
            ...
            let cas_result =
                bucket.compare_exchange_weak(head, new_entry, Ordering::Release, Ordering::Relaxed);
    ...
    fn remove(&self, key: &K) -> Result<Option<V>, MapError> {
        ...
        loop {
            ...
            let cas_result = if prev.is_null() {
                ...
                bucket.compare_exchange_weak(current, next, Ordering::Release, Ordering::Relaxed)
            } else {
    ...
    fn remove(&self, key: &K) -> Result<Option<V>, MapError> {
        ...
        loop {
            ...
            if valid {
                ...
                // In a production system, we'd defer the actual deallocation
                unsafe {
                    let _ = Box::from_raw(node_to_remove);
                }
                self.metrics.record_success();
                return Ok(old_value);
            }
...
mod tests {
    ...
    fn test_concurrent_hashmap_basic_operations() {
        ...
        // Test get
        assert_eq!(
            map.get(&"key1".to_string()).unwrap(),
            Some("value1".to_string())
        );
        assert_eq!(
            map.get(&"key2".to_string()).unwrap(),
        ...
        // Test update
        let old_value = map
            .insert("key1".to_string(), "new_value1".to_string())
            .unwrap();
    ...
    fn test_upsert_operation() {
        ...
        let result = map.upsert("key1".to_string(), "value1".to_string(), |v| {
            format!("updated_{}", v)
        });
        ...
        let result = map.upsert("key1".to_string(), "new_value".to_string(), |v| {
            format!("updated_{}", v)
        });
