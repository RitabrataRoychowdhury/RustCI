...
impl HttpAdapter {
    ...
    pub async fn new(config: AdapterConfig) -> Result<Self> {
        ...
        let base_url = config.custom.get("base_url")
            .and_then(|v| v.as_str())
    ...
    fn build_client(config: &AdapterConfig) -> Result<Client> {
        ...
        builder.build()
            .map_err(|e| ValkyrieError::AdapterInitializationFailed(format!("HTTP client build failed: {}", e)))
    }
    ...
    async fn message_to_request(&self, message: &AdapterMessage) -> Result<Request> {
        ...
        request_builder.build()
            .map_err(|e| ValkyrieError::MessageConversionFailed(format!("Request build failed: {}", e)))
    }
    
    /// Convert HTTP response to Valkyrie message
    async fn response_to_message(&self, response: Response, original_id: Uuid) -> Result<AdapterMessage> {
        ...
        let body_bytes = response.bytes().await
            .map_err(|e| ValkyrieError::MessageConversionFailed(format!("Response body read failed: {}", e)))?;
    ...
    async fn update_metrics(&self, operation: &str, latency: Duration, success: bool) {
        ...
        if success {
            ...
            if total_ops > 0 {
                metrics.error_rate = (metrics.error_rate * (total_ops - 1) as f64 + if success { 0.0 } else { 1.0 }) / total_ops as f64;
            }
        }
...
impl UniversalAdapter for HttpAdapter {
    async fn send(&self, message: AdapterMessage, qos: QoSParams) -> Result<SendResult> {
        ...
        debug!("Sending HTTP message: {} (type: {:?})", message.id, message.message_type);
        ...
        // Update metrics
        self.update_metrics("send", send_result.latency, send_result.success).await;
        
        if send_result.success {
            debug!("HTTP message sent successfully in {:?}", send_result.latency);
        } else {
    ...
    async fn initialize(&mut self) -> Result<()> {
        ...
        if matches!(health, HealthStatus::Unhealthy { .. }) {
            warn!("HTTP adapter initialized but health check failed: {}", health);
        }
...
impl RestTranslator {
    ...
    pub fn new() -> Self {
        ...
        default_headers.insert("User-Agent".to_string(), "Valkyrie-HTTP-Adapter/1.0".to_string());
                ...
                if let (Some(data), Some(encoding)) = (obj.get("data"), obj.get("encoding")) {
                    if encoding.as_str() == Some("base64") {
                        if let Some(base64_str) = data.as_str() {
                            return base64::decode(base64_str)
                                .map_err(|e| ValkyrieError::MessageConversionFailed(format!("Base64 decode failed: {}", e)));
                        }
                    }
                }
            }
        ...
        Self {
            default_config: AdapterConfig {
                ...
                custom: {
                    ...
                    custom.insert("base_url".to_string(), serde_json::Value::String("http://localhost:8080".to_string()));
                    custom
                },
            },
        }
