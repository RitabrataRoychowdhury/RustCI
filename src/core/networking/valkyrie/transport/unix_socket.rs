...
impl UnixSocketTransport {
    ...
    fn set_socket_permissions(&self, path: &PathBuf) -> Result<()> {
        #[cfg(unix)]
        {
            ...
            std::fs::set_permissions(path, permissions).map_err(|e| ProtocolError::ConnectionError {
...
impl Transport for UnixSocketTransport {
    ...
    async fn listen(&self, config: &TransportConfig) -> Result<Box<dyn Listener>> {
        let socket_path = match &config.transport_type {
            ...
            _ => return Err(ProtocolError::ConnectionError {
        ...
        let listener = UnixListener::bind(&socket_path).map_err(|e| ProtocolError::ConnectionError {
        ...
        Ok(Box::new(UnixSocketListener::new(listener, socket_path, self.unix_config.clone())))
    }
    ...
    async fn connect(&self, endpoint: &TransportEndpoint) -> Result<Box<dyn Connection>> {
        ...
        let stream = UnixStream::connect(&socket_path).await.map_err(|e| ProtocolError::ConnectionError {
...
impl Listener for UnixSocketListener {
    async fn accept(&mut self) -> Result<Box<dyn Connection>> {
        let (stream, _addr) = self.listener.accept().await.map_err(|e| ProtocolError::ConnectionError {
    ...
    async fn close(&mut self) -> Result<()> {
        ...
        if self.config.cleanup_on_shutdown && self.socket_path.exists() {
            std::fs::remove_file(&self.socket_path).map_err(|e| ProtocolError::ConnectionError {
                message: format!("Failed to cleanup socket file: {}", e)
            })?;
        }
        Ok(())
...
impl UnixSocketConnection {
    pub fn new(stream: UnixStream, socket_path: PathBuf, metrics: Arc<RwLock<TransportMetrics>>) -> Self {
    ...
    async fn receive_internal(&mut self) -> Result<ProtocolMessage> {
        ...
        if length > 64 * 1024 * 1024 { // 64MB limit for Unix sockets
