...
impl RbacManager {
    ...
    async fn initialize_defaults(&self) -> Result<()> {
        ...
        self.create_role("node".to_string(), "Basic node role".to_string())
            .await?;
        self.create_role("admin".to_string(), "Administrator role".to_string())
            .await?;
        self.create_role(
            "super_admin".to_string(),
            "Super administrator role".to_string(),
        )
        .await?;
        ...
        let permissions = vec![
            ...
            Permission::new(
                "admin.all".to_string(),
                "All administrative permissions".to_string(),
                "*".to_string(),
            )
        ...
        self.assign_permission_to_role("node".to_string(), "node.read".to_string())
            .await?;
        self.assign_permission_to_role("node".to_string(), "job.execute".to_string())
            .await?;

        self.assign_permission_to_role("admin".to_string(), "node.read".to_string())
            .await?;
        self.assign_permission_to_role("admin".to_string(), "node.write".to_string())
            .await?;
        self.assign_permission_to_role("admin".to_string(), "job.execute".to_string())
            .await?;
        self.assign_permission_to_role("admin".to_string(), "cluster.manage".to_string())
            .await?;

        self.assign_permission_to_role("super_admin".to_string(), "admin.all".to_string())
            .await?;

        // Set up role hierarchy
        self.add_role_parent("admin".to_string(), "node".to_string())
            .await?;
        self.add_role_parent("super_admin".to_string(), "admin".to_string())
            .await?;
    ...
    pub async fn assign_permission_to_role(
        ...
        if let Some(role) = roles.get_mut(&role_name) {
            ...
            Err(
                crate::error::AppError::SecurityError(format!("Role '{}' not found", role_name))
                    .into(),
            )
        }
    }
        ...
        Box::pin(async move {
            if depth > self.config.max_role_depth {
                return Err(crate::error::AppError::SecurityError(
                    "Role hierarchy too deep".to_string(),
                )
                .into());
            }
    ...
    pub async fn check_permission(
        ...
        let permission_granted = self
            .evaluate_permissions(&node_permissions, resource, action)
            .await?;
        ...
        metrics.average_check_time_ms = (metrics.average_check_time_ms
            * (metrics.total_authorization_checks - 1) as f64
            + duration.as_millis() as f64)
            / metrics.total_authorization_checks as f64;
        ...
        for permission_name in permissions {
            if let Some(permission) = permission_map.get(permission_name) {
                ...
                if permission.resource_type != "*"
                    && !resource.starts_with(&permission.resource_type)
                {
                    continue;
                }
                ...
                if permission.actions.contains("*") || permission.actions.contains(action) {
                    // Evaluate conditions
                    if self
                        .evaluate_conditions(&permission.conditions, &HashMap::new())
                        .await?
                    {
                        return Ok(true);
                    }
    ...
    async fn evaluate_policies(
        ...
        for policy in sorted_policies {
            ...
            for rule in &policy.rules {
                ...
                if !rule.actions.contains(&"*".to_string())
                ...
                if self
                    .evaluate_conditions(&rule.conditions, &subject.attributes)
                    .await?
                {
                    match rule.effect {
