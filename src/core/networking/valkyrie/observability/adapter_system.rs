...
impl ObservabilityManager {
    ...
    pub fn register_metrics_adapter(&mut self, adapter: Arc<dyn MetricsAdapter>) {
        ...
        self.circuit_breakers.insert(name, CircuitBreaker::new(self.config.circuit_breaker.clone()));
...
mod tests {
    ...
    async fn test_built_in_json_logger() {
        ...
        logger.log(LogLevel::Info, "Test message", &[("key", "value")]).await;
        logger.log_with_correlation(LogLevel::Warn, "Test with correlation", "test-id", &[]).await;
