...
use crate::domain::entities::{
    HealthStatus, Job, Runner, RunnerCapacity, RunnerEntity, RunnerId, RunnerStatus,
};
...
impl DefaultRunnerPoolManager {
    ...
    async fn schedule_job(&self, job: Job) -> Result<()> {
        ...
        if let Some(selected_runner_id) =
            self.load_balancer.select_runner(&context, &runners).await?
        {
                ...
                tokio::spawn(async move {
                    ...
                    match runner.execute(job).await {
                        Ok(_result) => {
                            ...
                            stats.avg_job_execution_time = (stats.avg_job_execution_time
                                * (stats.total_jobs_processed - 1) as f64
                                + execution_time)
    ...
    async fn update_stats(&self) -> Result<()> {
        ...
        if stats.total_runners > 0 {
            stats.pool_utilization =
                (stats.busy_runners as f64 / stats.total_runners as f64) * 100.0;
        }
    ...
    async fn start(&self) -> Result<()> {
        ...
        if *running {
            return Err(AppError::BadRequest(
                "Runner pool is already running".to_string(),
            ));
        }
            ...
            loop {
                ...
                while let Ok(Some(job)) = job_queue.dequeue().await {
                    ...
                    if !available_runners.is_empty() {
                        ...
                        if let Ok(Some(selected_runner_id)) =
                            load_balancer.select_runner(&context, &runners_read).await
                        {
                            if let Some(registration) = runners_read.get(&selected_runner_id) {
                                ...
                                tokio::spawn(async move {
                                    ...
                                    match runner.execute(job).await {
                                        Ok(_result) => {
                                            let execution_time =
                                                (Utc::now() - start_time).num_milliseconds() as f64
                                                    / 1000.0;
                                            info!(
                                                "Job {} completed successfully in {:.2}s",
                                                job_id, execution_time
                                            ...
                                            stats.total_jobs_processed += 1;
                                            stats.avg_job_execution_time = (stats
