...
use crate::core::runners::runner_pool::{LoadBalancer, RunnerRegistration, SchedulingContext};
use crate::domain::entities::{RunnerCapacity, RunnerId};
...
impl AffinityLoadBalancer {
    ...
    fn calculate_affinity_score(
        ...
        if let Some(required_type) = &context.job.requirements.runner_type {
            if std::mem::discriminant(&registration.entity.runner_type)
...
mod tests {
    ...
    use crate::domain::entities::{Job, JobRequirements, RunnerEntity, RunnerType};
    use std::collections::HashMap;
    use uuid::Uuid;
    ...
    fn create_test_context(
        available_runners: Vec<RunnerId>,
        capacities: HashMap<RunnerId, RunnerCapacity>,
    ) -> SchedulingContext {
        SchedulingContext {
