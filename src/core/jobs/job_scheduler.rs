...
use crate::domain::entities::{
    Job, JobId, JobResult, JobStatus, JobPriority, BackoffStrategy
};
...
impl JobScheduler for DefaultJobScheduler {
    ...
    async fn get_job_status(&self, job_id: JobId) -> Result<Option<JobStatus>> {
        ...
        let priority_queues = self.priority_queues.read().await;
        for queue in priority_queues.values() {
            if (queue.get_job(job_id).await?).is_some() {
                return Ok(Some(JobStatus::Queued));
            }
        }
    ...
    async fn start(&self) -> Result<()> {
        ...
        *running = true;
        ...
        // Clone the necessary components for background tasks
        
        // Priority queue processing task
        let _priority_queues = self.priority_queues.clone();
        let _runner_pool = self.runner_pool.clone();
        let config = self.config.clone();
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(
                tokio::time::Duration::from_millis(config.scheduling_interval_ms)
            );
            
            loop {
                interval.tick().await;
                // Process priority queues logic would go here
        ...
        let _scheduled_jobs = self.scheduled_jobs.clone();
        let config2 = self.config.clone();
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(
                tokio::time::Duration::from_millis(config2.scheduling_interval_ms)
            );
            
            loop {
                interval.tick().await;
                // Process scheduled jobs logic would go here
                // For now, just sleep to avoid busy loop
                tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
            }
        });
