...
use super::capability_detector::{DetectedCapabilities, PerformanceTier, PreferredProtocol};
use super::http_fallback::HttpFallbackSystem;
use super::unified_registry::{RegisteredRunner, RunnerType, UnifiedRunnerRegistry};
use super::valkyrie_adapter::ValkyrieRunnerAdapter;
use crate::domain::entities::{HealthStatus, Job, JobResult, RunnerCapacity, RunnerMetadata};
...
impl UnifiedRunnerInterface {
    ...
    pub async fn execute_job(&self, job: Job) -> Result<JobResult> {
        ...
        // Update metrics
        self.update_execution_metrics(&context, &result, start_time.elapsed())
            .await;
                ...
                Ok((runner_id, protocol)) => {
                    ...
                    match self.execute_with_protocol(context, &protocol).await {
                        ...
                        Err(e) => {
                            ...
                            if self.config.enable_protocol_migration
                                && attempt < self.config.max_retries
                            {
                                if let Some(fallback_protocol) =
                                    self.get_fallback_protocol(&protocol).await
                                {
                                    info!(
                                        "Attempting protocol migration from {:?} to {:?}",
    ...
    async fn select_optimal_runner_and_protocol(
        ...
        let available_runners = self
            .registry
            .list_runners_by_status(&super::unified_registry::RunnerStatus::Online)
            .await;
        ...
        match protocol {
            PreferredProtocol::Valkyrie => {
                if let Some(valkyrie_adapter) = &self.valkyrie_adapter {
                    ...
                    valkyrie_adapter
                        .as_ref()
                        .execute_domain_job(&context.job, context.runner_id)
                        .await
                } else {
    ...
    async fn get_fallback_protocol(
        &self,
        failed_protocol: &PreferredProtocol,
    ) -> Option<PreferredProtocol> {
    ...
    async fn record_successful_execution(
        ...
        // Update interface metrics
        {
            ...
            *metrics
            ...
            let total_time =
                metrics.avg_job_latency.as_nanos() as f64 * (metrics.total_jobs - 1) as f64;
            let new_avg =
                (total_time + execution_time.as_nanos() as f64) / metrics.total_jobs as f64;
            metrics.avg_job_latency = Duration::from_nanos(new_avg as u64);
        }

        // Update protocol performance
        self.selection_strategy
            .record_performance(context.runner_id, &context.protocol, execution_time, true)
            .await;
    }
    ...
    async fn record_failed_execution(&self, context: &JobExecutionContext, _error: &AppError) {
        ...
        // Update protocol performance
        self.selection_strategy
            .record_performance(context.runner_id, &context.protocol, execution_time, false)
            .await;
    }
...
impl ProtocolSelectionStrategy {
    ...
    pub async fn select_optimal_runner(
        ...
        match &self.algorithm {
            ...
            SelectionAlgorithm::HttpFirst => self.select_http_first(available_runners).await,
            SelectionAlgorithm::PerformanceBased => {
                self.select_performance_based(available_runners, context)
                    .await
            }
            SelectionAlgorithm::Adaptive => self.select_adaptive(available_runners, context).await,
            SelectionAlgorithm::RoundRobin => self.select_round_robin(available_runners).await,
    ...
    async fn select_valkyrie_first(
        &self,
        ...
        for runner in runners {
            if matches!(runner.runner_type, RunnerType::Valkyrie { .. })
                || matches!(
                    runner.runner_type,
                    RunnerType::Hybrid {
                        primary_protocol: PreferredProtocol::Valkyrie,
                        ..
                    }
                )
            {
    ...
    async fn select_adaptive(
        ...
        let requires_low_latency = context
    ...
    pub async fn record_performance(
        ...
        let perf = history
            .entry(runner_id)
            .or_insert_with(|| ProtocolPerformance {
        ...
        // Update success rate
        perf.success_rate =
            (perf.total_operations - perf.failed_operations) as f64 / perf.total_operations as f64;
...
mod tests {
    ...
    fn test_performance_requirements_extraction() {
        ...
        job.metadata
            .insert("max_latency".to_string(), "100".to_string());
        job.metadata
            .insert("cpu_cores".to_string(), "4".to_string());
