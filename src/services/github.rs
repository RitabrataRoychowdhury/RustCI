...
use crate::{
    ...
    models::{GitHubRepo, GitHubContent, GitHubBranch, GitHubCommit, GitHubPullRequest, GitHubUser, PullRequestRequest},
};
...
pub trait GitHubService: Send + Sync {
    ...
    async fn get_repository_contents(&self, access_token: &str, owner: &str, repo: &str, path: &str) -> Result<Vec<GitHubContent>>;
    async fn check_dockerfile_exists(&self, access_token: &str, owner: &str, repo: &str) -> Result<bool>;
    async fn create_branch(&self, access_token: &str, owner: &str, repo: &str, branch_name: &str, from_branch: &str) -> Result<GitHubBranch>;
    async fn create_file(&self, access_token: &str, owner: &str, repo: &str, path: &str, content: &str, message: &str, branch: &str) -> Result<GitHubCommit>;
    async fn create_pull_request(&self, access_token: &str, pr_request: &PullRequestRequest) -> Result<GitHubPullRequest>;
    async fn delete_branch(&self, access_token: &str, owner: &str, repo: &str, branch_name: &str) -> Result<()>;
}
...
impl GitHubServiceImpl {
        ...
        T: serde::de::DeserializeOwned,
    {
        ...
        response
            .json::<T>()
            .await
            .map_err(|e| AppError::GitHubApiError(format!("Failed to parse GitHub response: {}", e)))
    }
}
...
impl GitHubService for GitHubServiceImpl {
    ...
    async fn get_repository_contents(&self, access_token: &str, owner: &str, repo: &str, path: &str) -> Result<Vec<GitHubContent>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| AppError::GitHubApiError(format!("Failed to get repository contents: {}", e)))?;
    ...
    async fn check_dockerfile_exists(&self, access_token: &str, owner: &str, repo: &str) -> Result<bool> {
        let url = format!("{}/repos/{}/{}/contents/Dockerfile", self.base_url, owner, repo);
        let headers = self.create_headers(access_token);
    ...
    async fn create_branch(&self, access_token: &str, owner: &str, repo: &str, branch_name: &str, from_branch: &str) -> Result<GitHubBranch> {
        ...
        let ref_url = format!("{}/repos/{}/{}/git/ref/heads/{}", self.base_url, owner, repo, from_branch);
        let headers = self.create_headers(access_token);
        ...
        let ref_response = self
            ...
            .send()
            .await
            .map_err(|e| AppError::GitHubApiError(format!("Failed to get branch reference: {}", e)))?;
        ...
        Ok(GitHubBranch {
            ...
            commit: GitHubCommit {
                sha: sha.to_string(),
                url: format!("{}/repos/{}/{}/git/commits/{}", self.base_url, owner, repo, sha),
                ...
                tree: crate::models::GitHubTree {
                    sha: sha.to_string(),
                    url: format!("{}/repos/{}/{}/git/trees/{}", self.base_url, owner, repo, sha),
                },
                parents: vec![],
            },
            protected: false,
    ...
    async fn create_file(&self, access_token: &str, owner: &str, repo: &str, path: &str, content: &str, message: &str, branch: &str) -> Result<GitHubCommit> {
        ...
        Ok(GitHubCommit {
            ...
            tree: crate::models::GitHubTree {
                sha: commit_data["tree"]["sha"].as_str().unwrap_or("").to_string(),
                url: commit_data["tree"]["url"].as_str().unwrap_or("").to_string(),
            },
            parents: vec![],
        })
    }
    ...
    async fn create_pull_request(&self, access_token: &str, pr_request: &PullRequestRequest) -> Result<GitHubPullRequest> {
        let url = format!("{}/repos/{}/{}/pulls", self.base_url, pr_request.owner, pr_request.repo);
        let headers = self.create_headers(access_token);
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| AppError::GitHubApiError(format!("Failed to create pull request: {}", e)))?;
        
        self.handle_github_response(response).await
    }
    
    async fn delete_branch(&self, access_token: &str, owner: &str, repo: &str, branch_name: &str) -> Result<()> {
        let url = format!("{}/repos/{}/{}/git/refs/heads/{}", self.base_url, owner, repo, branch_name);
        let headers = self.create_headers(access_token);
