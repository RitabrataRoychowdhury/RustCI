...
use crate::infrastructure::runners::{
    ValkyrieRunnerAdapter, ValkyrieRunnerTrait,
};
use crate::infrastructure::runners::valkyrie_adapter::{
    ValkyrieJob, JobType, JobPriority, JobPayload, JobRequirements, JobMetadata, AdapterPerformanceSnapshot,
};
use crate::presentation::routes::valkyrie::{
    EnhancedJobSubmissionRequest, EnhancedJobSubmissionResponse, EnhancedJobStatusResponse,
    EnhancedRunnerListResponse, EnhancedRunnerInfo, SystemHealthResponse, IntegrationStatusResponse,
    JobListQuery, RunnerListQuery, ComponentHealthResponse,
};
...
impl ValkyrieControlPlaneHandler {
    ...
    pub async fn submit_job_enhanced(
        ...
        let submission_result = if let Some(adapter) = &self.valkyrie_adapter {
            ...
            adapter.submit_job(valkyrie_job).await
        ...
        let response = EnhancedJobSubmissionResponse {
            ...
            estimated_start_time: Some(submission_result.estimated_start_time.elapsed().as_secs().to_string()),
            estimated_completion_time: Some(submission_result.estimated_completion_time.elapsed().as_secs().to_string()),
            ...
            routing_path: submission_result.routing_path.as_ref().map(|route| {
                crate::presentation::routes::valkyrie::RoutingPathResponse {
                    ...
                    hops: route.hops.iter().map(|h| h.to_string()).collect(),
                    estimated_latency: Some(route.estimated_latency.as_micros() as u64),
                    protocol_used: "valkyrie".to_string(),
                }
            }),
            performance_metrics: Some(crate::presentation::routes::valkyrie::JobPerformanceResponse {
    ...
    pub async fn list_runners_enhanced(
        ...
        if let Some(adapter) = &self.valkyrie_adapter {
            let stats = adapter.get_runner_pool_stats().await
                .map_err(|e| AppError::InternalError { 
                    component: "valkyrie_control_plane".to_string(),
    ...
    pub async fn update_runner_status_enhanced(
        ...
        if let Some(_adapter) = &self.valkyrie_adapter {
            ...
            self.update_runner_status_legacy_fallback(runner_id, status).await
        }
    }
    ...
    pub async fn get_performance_metrics(&self) -> Result<Value> {
        ...
        // Refresh metrics
        if let Some(adapter) = &self.valkyrie_adapter {
            let adapter_metrics = adapter.get_performance_metrics().await
                .map_err(|e| AppError::InternalError { 
                    component: "valkyrie_control_plane".to_string(),
    ...
    pub async fn get_system_health(&self) -> Result<SystemHealthResponse> {
        ...
        let mut components = vec![
            ComponentHealthResponse {
                ...
                status: if integration_status.valkyrie_enabled { "healthy" } else { "disabled" }.to_string(),
                message: Some(format!("Mode: {:?}", integration_status.integration_mode)),
                last_check: SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
        ...
        Ok(SystemHealthResponse {
            ...
            valkyrie_status: if integration_status.valkyrie_enabled { "enabled" } else { "disabled" }.to_string(),
            control_plane_status: "operational".to_string(),
            runner_pool_status: "operational".to_string(),
            performance_status: if integration_status.performance_enhancement_active { "enhanced" } else { "standard" }.to_string(),
            components,
    ...
    pub async fn update_valkyrie_config(&self, new_config: Value) -> Result<()> {
        ...
        self.integration_service.validate_config(&new_config).await?;
    ...
    pub async fn toggle_fallback_mode(&self, request: Value) -> Result<Value> {
        let enable_fallback = request.get("enable")
    ...
    async fn convert_to_valkyrie_job(&self, request: EnhancedJobSubmissionRequest) -> Result<ValkyrieJob> {
        ...
        let requirements = JobRequirements {
            ...
            max_execution_time: request.requirements.as_ref()
                .and_then(|r| r.max_execution_time)
                .map(Duration::from_secs),
            required_capabilities: request.requirements.as_ref()
                .and_then(|r| r.required_capabilities.clone())
                .unwrap_or_default(),
            preferred_regions: request.requirements.as_ref()
        ...
        let metadata = JobMetadata {
            user_id: request.metadata.as_ref().and_then(|m| m.get("user_id").cloned()),
            project_id: request.metadata.as_ref().and_then(|m| m.get("project_id").cloned()),
            pipeline_id: request.metadata.as_ref().and_then(|m| m.get("pipeline_id").cloned()),
            stage: request.metadata.as_ref().and_then(|m| m.get("stage").cloned()),
    ...
    fn convert_qos_guarantees(&self, guarantees: &crate::infrastructure::runners::valkyrie_adapter::QoSGuarantees) -> crate::presentation::routes::valkyrie::QoSGuaranteesResponse {
        crate::presentation::routes::valkyrie::QoSGuaranteesResponse {
            ...
            priority_level: guarantees.priority_level.as_ref().map(|p| format!("{:?}", p)),
        }
    }
    
    async fn convert_job_status_to_enhanced(&self, job_id: Uuid, status: crate::infrastructure::runners::valkyrie_adapter::JobStatus) -> Result<EnhancedJobStatusResponse> {
        ...
        let (status_str, started_at, completed_at, runner_id, result, error) = match status {
            ...
            JobStatus::Dispatched { runner_id, dispatched_at: _ } => ("dispatched", None, None, Some(runner_id), None, None),
            JobStatus::Running { runner_id, started_at } => ("running", Some(started_at), None, Some(runner_id), None, None),
            JobStatus::Completed { result, completed_at } => ("completed", None, Some(completed_at), None, Some(result), None),
            JobStatus::Failed { error, failed_at } => ("failed", None, Some(failed_at), None, None, Some(error)),
            JobStatus::Cancelled { cancelled_at } => ("cancelled", None, Some(cancelled_at), None, None, None),
            JobStatus::Timeout { timeout_at } => ("timeout", None, Some(timeout_at), None, None, Some("Job timed out".to_string())),
        };
        ...
        Ok(EnhancedJobStatusResponse {
            ...
            created_at: SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
                .unwrap_or_default().as_secs().to_string(),
            started_at: started_at.map(|_| SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
                .unwrap_or_default().as_secs().to_string()),
            completed_at: completed_at.map(|_| SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
                .unwrap_or_default().as_secs().to_string()),
            runner_id: runner_id.map(|id| id.to_string()),
            result: result.map(|r| crate::presentation::routes::valkyrie::JobResultResponse {
    ...
    async fn submit_job_legacy_fallback(&self, job: ValkyrieJob) -> Result<EnhancedJobSubmissionResponse> {
    ...
    async fn get_job_status_legacy_fallback(&self, job_id: Uuid) -> Result<EnhancedJobStatusResponse> {
    ...
    async fn list_runners_legacy_fallback(&self, _query: RunnerListQuery) -> Result<EnhancedRunnerListResponse> {
    ...
    async fn update_runner_status_legacy_fallback(&self, _runner_id: Uuid, _status: Value) -> Result<()> {
