...
use crate::{
    domain::entities::ValidationResult,
...
impl DockerfileValidatorContext {
    ...
    pub async fn validate(
        ...
        self.strategy
            .validate(dockerfile_content, context_path)
            .await
    }
...
impl LocalDockerValidator {
    pub fn new() -> Result<Self> {
        let docker = Docker::connect_with_socket_defaults().map_err(|e| {
            AppError::DockerValidationError(format!("Failed to connect to Docker: {}", e))
        })?;
    ...
    async fn create_build_context(
        ...
        let temp_dir = tempfile::tempdir().map_err(|e| {
            AppError::DockerValidationError(format!("Failed to create temp directory: {}", e))
        })?;

        // Write Dockerfile
        let dockerfile_path = temp_dir.path().join("Dockerfile");
        let mut dockerfile_file = fs::File::create(&dockerfile_path).await.map_err(|e| {
            AppError::DockerValidationError(format!("Failed to create Dockerfile: {}", e))
        })?;
        ...
        dockerfile_file
            ...
            .await
            .map_err(|e| {
                AppError::DockerValidationError(format!("Failed to write Dockerfile: {}", e))
            })?;
        ...
        if let Some(_context) = context_path {
            ...
            fs::write(
                &readme_path,
                "# Test Project\nThis is a test context for Dockerfile validation.",
            )
            .await
            .map_err(|e| {
                AppError::DockerValidationError(format!("Failed to create context file: {}", e))
            })?;
        }
    ...
    async fn build_image(&self, build_context: &TempDir) -> Result<(String, Vec<String>)> {
        ...
        let mut build_stream =
            self.docker_client
                .build_image(build_options, None, Some(build_context_tar.into()));

        while let Some(build_info) = build_stream
            .try_next()
            .await
            .map_err(|e| AppError::DockerValidationError(format!("Build stream error: {}", e)))?
        {
            ...
            if let Some(error) = build_info.error {
                return Err(AppError::DockerValidationError(format!(
                    "Build failed: {}",
                    error
                )));
            }
    ...
    async fn test_container_run(&self, image_tag: &str) -> Result<Vec<String>> {
        ...
        let container = self
            ...
            .await
            .map_err(|e| {
                AppError::DockerValidationError(format!("Failed to create container: {}", e))
            })?;
        ...
        self.docker_client
            ...
            .await
            .map_err(|e| {
                AppError::DockerValidationError(format!("Failed to start container: {}", e))
            })?;
    ...
    async fn create_tar_archive(&self, dir_path: &std::path::Path) -> Result<Vec<u8>> {
        let mut tar_data = Vec::new();
        {
            ...
            tar.append_dir_all(".", dir_path).map_err(|e| {
                AppError::DockerValidationError(format!("Failed to create tar archive: {}", e))
            })?;
            tar.finish().map_err(|e| {
                AppError::DockerValidationError(format!("Failed to finish tar archive: {}", e))
            })?;
        }
        Ok(tar_data)
    }
...
impl DockerfileValidator for LocalDockerValidator {
    async fn validate(
        ...
        let build_context = self
            .create_build_context(dockerfile_content, context_path)
            .await?;
        ...
        let success = !build_logs
            .iter()
            .any(|log| log.to_lowercase().contains("error"))
            && !run_logs.iter().any(|log| log.contains("failed"));

        let mut warnings = Vec::new();
        if build_logs
            .iter()
            .any(|log| log.to_lowercase().contains("warning"))
        {
...
impl DockerfileValidator for RemoteDockerValidator {
    async fn validate(
        ...
        Err(AppError::DockerValidationError(
            "Remote validation not yet implemented".to_string(),
        ))
    }
...
impl DockerfileValidatorFactory {
    ...
    pub fn create_remote_validator(
        endpoint: String,
        api_key: Option<String>,
    ) -> Box<dyn DockerfileValidator> {
        Box::new(RemoteDockerValidator::new(endpoint, api_key))
    }

    pub fn create_validator_by_type(
        validator_type: ValidatorType,
    ) -> Result<Box<dyn DockerfileValidator>> {
...
impl Default for DockerfileValidationService {
    fn default() -> Self {
        Self::new()
    }
}
...
impl DockerfileValidationService {
    ...
    pub async fn validate_dockerfile(
        ...
        let fallback_validator =
            DockerfileValidatorFactory::create_validator_by_type(self.fallback_validator.clone())?;
    ...
    pub async fn validate_with_multiple_strategies(
        ...
        for strategy in strategies {
            let validator_result =
                DockerfileValidatorFactory::create_validator_by_type(strategy.clone());
...
mod tests {
    ...
    async fn test_mock_validator_failure() {
        ...
        let result = validator
            .validate("INVALID DOCKERFILE", None)
            .await
            .unwrap();
    ...
    async fn test_validation_service() {
        ...
        let result = service
            .validate_dockerfile("FROM ubuntu:20.04", None, None)
            .await
            .unwrap();
        assert!(result.success);
    }
    ...
    async fn test_multiple_strategies() {
        ...
        let results = service
            .validate_with_multiple_strategies("FROM ubuntu:20.04", None, strategies)
            .await
            .unwrap();
