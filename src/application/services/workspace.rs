...
use crate::{
    application::services::EncryptionService,
    domain::entities::{GitHubUser, RepositoryMetadata, Workspace, WorkspaceUpdate},
...
pub trait WorkspaceService: Send + Sync {
    async fn create_or_get_workspace(
        &self,
        user_id: Uuid,
...
impl WorkspaceService for WorkspaceServiceImpl {
    async fn create_or_get_workspace(
        ...
        if let Some(existing_workspace) = self
            .workspace_repo
            .find_by_github_user_id(github_user.id)
            .await?
        {
            ...
            if existing_workspace.user_id != user_id {
                ...
                return self
                    .workspace_repo
                    .update(existing_workspace.id, updates)
                    .await;
            }
            return Ok(existing_workspace);
        }
    ...
    async fn get_decrypted_token(&self, workspace_id: Uuid) -> Result<String> {
        ...
        if workspace.encrypted_github_token.is_empty() {
            return Err(AppError::AuthenticationError(
                "No GitHub token stored for workspace".to_string(),
            ));
        }

        self.encryption_service
            .decrypt(&workspace.encrypted_github_token)
            .await
    }
    ...
    async fn add_repository(
        ...
        if repositories.iter().any(|r| r.id == repo_metadata.id) {
            return Err(AppError::ValidationError(
                "Repository already linked to workspace".to_string(),
            ));
        }
    ...
    async fn remove_repository(&self, workspace_id: Uuid, repository_id: i64) -> Result<Workspace> {
        ...
        let repositories: Vec<RepositoryMetadata> = workspace
            .repositories
            .into_iter()
        ...
        if repositories.len() == original_len {
            return Err(AppError::RepositoryNotFound(
                "Repository not found in workspace".to_string(),
            ));
        }
    ...
    async fn update_repository(
        ...
        workspace_id: Uuid,
        repository_id: i64,
        updated_repo: RepositoryMetadata,
    ) -> Result<Workspace> {
        let workspace = self
            .workspace_repo
            .find_by_id(workspace_id)
        ...
        let repo_index = repositories
            .iter()
            .position(|r| r.id == repository_id)
            .ok_or_else(|| {
                AppError::RepositoryNotFound("Repository not found in workspace".to_string())
            })?;
...
mod tests {
    ...
    impl WorkspaceRepository for MockWorkspaceRepository {
        ...
        async fn find_by_github_user_id(&self, github_user_id: i64) -> Result<Option<Workspace>> {
            ...
            Ok(workspaces
                .iter()
                .find(|w| w.github_user_id == github_user_id)
                .cloned())
        }
    }
