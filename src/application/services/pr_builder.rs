...
use crate::{
    domain::entities::{ProjectType, PullRequestRequest},
...
impl DockerfilePRBuilder {
    ...
    pub fn build(self) -> Result<PullRequestRequest> {
        ...
        let title = format!(
            "feat: Add auto-generated Dockerfile for {} project",
...
impl TemplatePRBuilder {
    ...
    pub fn build(self) -> Result<PullRequestRequest> {
        ...
        self.base_builder.title(title).body(body).build()
    }
    ...
    fn render_feature_template(&self) -> Result<(String, String)> {
        let feature_name = self.template_data.get("feature_name").ok_or_else(|| {
            AppError::ValidationError("feature_name is required for feature template".to_string())
        })?;
    ...
    fn render_bugfix_template(&self) -> Result<(String, String)> {
        let bug_description = self.template_data.get("bug_description").ok_or_else(|| {
            AppError::ValidationError("bug_description is required for bugfix template".to_string())
        })?;
    ...
    fn render_security_template(&self) -> Result<(String, String)> {
        let security_issue = self.template_data.get("security_issue").ok_or_else(|| {
            AppError::ValidationError(
                "security_issue is required for security template".to_string(),
            )
        })?;
...
mod tests {
    ...
    fn test_pr_builder_validation() {
        let result = PullRequestBuilder::new().title("Test PR").build();
    ...
    fn test_pr_builder_factory() {
        let pr_request =
            PRBuilderFactory::create_dockerfile_pr("testuser", "testrepo", ProjectType::Go, true)
                .unwrap();
