...
use crate::core::networking::transport::{
    BufferConfig, TimeoutConfig, TransportConfig, TransportType,
};
use crate::core::networking::valkyrie::engine::{
    AuditConfig, AuthMethod, AuthenticationConfig, AuthorizationConfig, CipherSuite,
    ConnectionPoolConfig, ConnectionReuseStrategy, EncryptionConfig, FeatureFlags,
    FlowControlConfig, LoadBalancingStrategy, LogFormat, LoggingConfig, MetricsConfig,
    ObservabilityConfig, PerformanceConfig, ProtocolConfig, ProtocolTimeouts, RoutingConfig,
    SecurityConfig, StreamingConfig, TracingConfig, TransportLayerConfig,
    TransportSelectionStrategy, ValkyrieConfig, ValkyrieEngine,
};

use crate::error::{AppError, Result};
use crate::infrastructure::runners::{
    ValkyrieAdapterConfig, ValkyrieRunnerAdapter, ValkyrieRunnerTrait,
};
...
impl ValkyrieIntegrationService {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        let valkyrie_adapter = self
            .initialize_valkyrie_adapter(&config, valkyrie_engine)
            .await?;
    ...
    async fn initialize_valkyrie_adapter(
        ...
        // Initialize adapter
        let adapter = ValkyrieRunnerAdapter::new(valkyrie_engine, adapter_config)
            .await
    ...
    fn is_restart_required(
        &self,
        old_config: &ValkyrieIntegrationConfig,
        new_config: &ValkyrieIntegrationConfig,
    ) -> bool {
            ...
            validator: Box::new(|config| {
                if config.valkyrie_enabled && config.valkyrie_listen_address.is_empty() {
                    return Err(AppError::ValidationError(
                        "Valkyrie listen address must be specified when Valkyrie is enabled"
                            .to_string(),
                    ));
                }
                Ok(())
