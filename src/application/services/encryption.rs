...
impl AesGcmEncryptionService {
    ...
    pub fn from_env() -> Result<Self> {
        let key_base64 = std::env::var("ENCRYPTION_KEY").map_err(|_| {
            AppError::EncryptionError("ENCRYPTION_KEY environment variable not set".to_string())
        })?;
...
impl EncryptionService for AesGcmEncryptionService {
    ...
    async fn decrypt(&self, ciphertext_base64: &str) -> Result<String> {
        ...
        String::from_utf8(plaintext).map_err(|e| {
            AppError::EncryptionError(format!("Invalid UTF-8 in decrypted data: {}", e))
        })
    }
}
