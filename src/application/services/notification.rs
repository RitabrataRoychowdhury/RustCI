...
use crate::{
    domain::entities::{GitHubPullRequest, ProjectType, ValidationResult},
...
pub enum EventData {
    ...
    PullRequestCreated {
        pull_request: Box<GitHubPullRequest>,
        dockerfile_content: String,
    },
    WorkspaceCreated {
        workspace_id: Uuid,
...
pub struct EventPublisher {
    ...
    event_history: Arc<std::sync::RwLock<Vec<Event>>>,
...
impl EventPublisher {
    pub fn new() -> Self {
        Self {
            observers: Vec::new(),
            event_history: Arc::new(std::sync::RwLock::new(Vec::new())),
    ...
    pub async fn publish(&self, event: Event) -> Result<()> {
        tracing::info!(
            "Publishing event: {:?} for workspace: {}",
            event.event_type,
            event.workspace_id
        );
        ...
        if !notification_errors.is_empty() {
            tracing::warn!(
                "Some observers failed to handle event: {}",
...
impl Default for DockerfileValidationObserver {
    fn default() -> Self {
        Self::new()
    }
}
        ...
        match &event.data {
            EventData::DockerfileValidated {
                ...
                if validation_result.success {
                    tracing::info!(
                        "✅ Dockerfile validation successful for generation: {}",
                        generation_id
                    );
                    // Could send success notification to user, update database, etc.
                } else {
                    tracing::warn!(
                        "❌ Dockerfile validation failed for generation: {}",
                        generation_id
                    );
            ...
            EventData::ValidationStarted {
                generation_id,
                validator_type,
            } => {
                tracing::info!(
                    "🔄 Dockerfile validation started for generation: {} using {}",
                    generation_id,
                    validator_type
                );
            }
            EventData::ValidationCompleted {
                generation_id,
                success,
                duration_ms,
            } => {
                tracing::info!(
                    "✅ Dockerfile validation completed for generation: {} in {}ms (success: {})",
                    generation_id,
                    duration_ms,
                    success
                );
            ...
            _ => {
                tracing::debug!(
                    "DockerfileValidationObserver received unhandled event type: {:?}",
                    event.event_type
                );
            }
        }
...
impl Default for PullRequestObserver {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Observer for PullRequestObserver {
    async fn notify(&self, event: &Event) -> Result<()> {
        match &event.data {
            ...
            _ => {
                tracing::debug!(
                    "PullRequestObserver received unhandled event type: {:?}",
                    event.event_type
                );
            }
        }
...
impl Observer for WorkspaceObserver {
    async fn notify(&self, event: &Event) -> Result<()> {
        match &event.data {
            EventData::WorkspaceCreated {
                workspace_id,
                github_username,
            } => {
                tracing::info!(
                    "🏗️ Workspace created for user: {} (ID: {})",
                    github_username,
                    workspace_id
                );
                // Could initialize default settings, send welcome message, etc.
            }
            EventData::WorkspaceUpdated {
                workspace_id,
                changes,
            } => {
                tracing::info!(
                    "🔄 Workspace updated: {} - Changes: {:?}",
                    workspace_id,
                    changes
                );
                // Could log changes, notify team members, etc.
            }
            EventData::RepositoryLinked {
                repository_name,
                repository_id,
                project_type,
            } => {
                tracing::info!(
                    "🔗 Repository linked: {} (ID: {}) - Type: {:?}",
                    repository_name,
                    repository_id,
                    project_type
                );
            ...
            _ => {
                tracing::debug!(
                    "WorkspaceObserver received unhandled event type: {:?}",
                    event.event_type
                );
            }
        }
...
impl Observer for ErrorObserver {
    async fn notify(&self, event: &Event) -> Result<()> {
        match &event.data {
            ...
            _ => {
                tracing::debug!(
                    "ErrorObserver received unhandled event type: {:?}",
                    event.event_type
                );
            }
        }
...
impl NotificationService {
    pub fn new() -> Self {
        Self {
            publisher: Arc::new(std::sync::RwLock::new(
                ObserverFactory::create_default_publisher(),
            )),
        }
    }
        ...
        let publisher = {
            if let Ok(publisher) = self.publisher.read() {
                ...
                return Err(AppError::InternalServerError(
                    "Failed to acquire publisher lock".to_string(),
                ));
            }
        };
