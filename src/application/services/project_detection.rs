...
use crate::{
    domain::entities::{GitHubContent, ProjectType},
...
impl ProjectTypeDetector for RustProjectDetector {
    fn detect(&self, files: &[GitHubContent]) -> Result<ProjectType> {
        ...
        let has_src_dir = files
            .iter()
            .any(|f| f.name == "src" && f.file_type == "dir");
        let has_rust_files = files.iter().any(|f| f.name.ends_with(".rs"));

        if has_cargo_toml && (has_src_dir || has_rust_files) {
            Ok(ProjectType::Rust)
        } else {
            Err(AppError::ProjectTypeDetectionFailed(
                "Not a Rust project".to_string(),
            ))
        }
    }
...
impl Default for NodeProjectDetector {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ProjectTypeDetector for NodeProjectDetector {
    fn detect(&self, files: &[GitHubContent]) -> Result<ProjectType> {
        ...
        let has_node_modules = files
            .iter()
            .any(|f| f.name == "node_modules" && f.file_type == "dir");
        let has_js_files = files
            .iter()
            .any(|f| f.name.ends_with(".js") || f.name.ends_with(".ts"));
        let has_yarn_lock = files.iter().any(|f| f.name == "yarn.lock");
        let has_package_lock = files.iter().any(|f| f.name == "package-lock.json");

        if has_package_json
            && (has_node_modules || has_js_files || has_yarn_lock || has_package_lock)
        {
            Ok(ProjectType::Node)
        } else {
            Err(AppError::ProjectTypeDetectionFailed(
                "Not a Node.js project".to_string(),
            ))
        }
    }
...
impl Default for PythonProjectDetector {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ProjectTypeDetector for PythonProjectDetector {
    fn detect(&self, files: &[GitHubContent]) -> Result<ProjectType> {
        ...
        let has_venv = files
            .iter()
            .any(|f| (f.name == "venv" || f.name == ".venv") && f.file_type == "dir");

        if (has_requirements_txt || has_setup_py || has_pyproject_toml || has_pipfile || has_venv)
            && has_python_files
        {
            ...
            Err(AppError::ProjectTypeDetectionFailed(
                "Not a Python project".to_string(),
            ))
        }
    }
...
impl Default for JavaProjectDetector {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ProjectTypeDetector for JavaProjectDetector {
    fn detect(&self, files: &[GitHubContent]) -> Result<ProjectType> {
        ...
        let has_build_gradle = files
            .iter()
            .any(|f| f.name == "build.gradle" || f.name == "build.gradle.kts");
        ...
        if (has_pom_xml || has_build_gradle || has_gradle_wrapper)
            && (has_java_files || has_src_main)
        {
            Ok(ProjectType::Java)
        } else {
            Err(AppError::ProjectTypeDetectionFailed(
                "Not a Java project".to_string(),
            ))
        }
    }
...
impl Default for GoProjectDetector {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Default for UnknownProjectDetector {
    fn default() -> Self {
        Self::new()
    }
}
...
pub fn analyze_project_files(files: &[GitHubContent]) -> ProjectAnalysis {
    ...
    for file in files {
        match file.name.as_str() {
            "Cargo.toml" => analysis
                .rust_indicators
                .push("Cargo.toml found".to_string()),
            "package.json" => analysis
                .node_indicators
                .push("package.json found".to_string()),
            "requirements.txt" => analysis
                .python_indicators
                .push("requirements.txt found".to_string()),
        ...
        if file.name.ends_with(".rs") {
            analysis
                .rust_indicators
                .push(format!("Rust file: {}", file.name));
        } else if file.name.ends_with(".js") || file.name.ends_with(".ts") {
            analysis
                .node_indicators
                .push(format!("JavaScript/TypeScript file: {}", file.name));
        } else if file.name.ends_with(".py") {
            analysis
                .python_indicators
                .push(format!("Python file: {}", file.name));
        } else if file.name.ends_with(".java") {
            analysis
                .java_indicators
                .push(format!("Java file: {}", file.name));
        } else if file.name.ends_with(".go") {
            analysis
                .go_indicators
                .push(format!("Go file: {}", file.name));
        }
    }
...
mod tests {
    use super::*;
    use crate::domain::entities::GitHubContent;
