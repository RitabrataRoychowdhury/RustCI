...
impl DockerConnector {
    ...
    async fn validate_docker_available(&self) -> Result<()> {
        ...
        let output = Command::new("docker")
            ...
            .map_err(|e| AppError::ConnectorConfigError(
                format!("Docker not found on system: {}", e)
            ))?;
    ...
    async fn build_image_if_needed(
        ...
        if let Some(dockerfile) = &step.config.dockerfile {
            let image_name = step.config.image.clone()
            ...
            let build_context = step.config.build_context.as_ref()
            ...
            Ok(step.config.image.clone().unwrap_or_else(|| "ubuntu:latest".to_string()))
        }
    }
    ...
    async fn execute_in_container(
        ...
        info!("🐳 Running container: {} with image: {}", container_name, image);
        ...
        if output.status.success() {
            ...
            warn!("⚠️ Container execution failed with exit code: {}", exit_code);
            debug!("📤 Error: {}", stderr);
        }
...
impl Connector for DockerConnector {
    async fn execute_step(
        ...
        let command = step.config.command.as_ref()
            ...
            .ok_or_else(|| AppError::ValidationError(
                "Docker step requires command or script".to_string()
            ))?;

        // Execute in container
        let result = self.execute_in_container(
            &image,
            command,
            workspace,
            env,
        ...
        if result.is_success() {
            ...
            error!("❌ Docker step '{}' failed with exit code: {}", step.name, result.exit_code);
        }
    ...
    async fn post_execute(&self, step: &Step, result: &ExecutionResult) -> Result<()> {
        debug!("🏁 Docker post-execution hook for step: {} (exit_code: {})", 
        ...
        if result.is_success() {
            ...
            warn!("⚠️ Docker step '{}' failed with exit code: {}", step.name, result.exit_code);
        }
