...
mod integration_tests {
    use crate::ci::{
        config::{
            CIPipeline, Stage, Step, StepConfig, StepType, Trigger, TriggerConfig, TriggerType,
        },
        connectors::{
            ConnectorManager,
            traits::{ConnectorType, ExecutionResult},
        },
        workspace::Workspace,
    };    
    ...
    async fn test_connector_validation_integration() {
        ...
        let docker_connector = connector_manager
        ...
        let k8s_connector = connector_manager
    ...
    async fn test_custom_connector_type_determination() {
        ...
        let determined_type = connector_manager
            .determine_connector_type(&custom_step)
            .unwrap();
        assert_eq!(
            determined_type,
            ConnectorType::Custom("my-custom-plugin".to_string())
        );
    ...
    async fn test_connector_error_handling() {
        ...
        let result =
            connector_manager.get_connector(ConnectorType::Custom("unsupported".to_string()));
        assert!(result.is_err());
    ...
    async fn test_connector_manager_stats_and_management() {
        ...
        let _docker = connector_manager
            .get_connector(ConnectorType::Docker)
            .unwrap();
        let _k8s = connector_manager
            .get_connector(ConnectorType::Kubernetes)
            .unwrap();
    ...
    async fn test_execution_result_comprehensive() {
        ...
        let custom_result =
            ExecutionResult::new(2, "custom output".to_string(), "custom error".to_string());
        ...
        assert_eq!(
            result_with_metadata.metadata.get("key1"),
            Some(&"value1".to_string())
        );
        assert_eq!(
            result_with_metadata.metadata.get("key2"),
    ...
    async fn test_connector_system_end_to_end() {
        ...
        for stage in &pipeline.stages {
            for step in &stage.steps {
                ...
                assert!(
                    pre_hook_result.is_ok(),
                    "Pre-hook failed for step: {}",
                    step.name
                );
                ...
                assert!(
                    post_hook_result.is_ok(),
                    "Post-hook failed for step: {}",
                    step.name
                );
            }
        }
        ...
        // Cleanup workspace
        let cleanup_result = workspace_manager
            .cleanup_workspace(workspace.execution_id)
            .await;
    ...
    async fn test_connector_system_performance() {
        ...
        for _ in 0..100 {
            let _connector = connector_manager
                .get_connector(ConnectorType::Docker)
                .unwrap();
        }
    ...
    async fn test_connector_system_memory_usage() {
        ...
        for connector_type in &connector_types {
            let _connector = connector_manager
                .get_connector(connector_type.clone())
                .unwrap();
        }
