...
mod tests {
    ...
    use crate::ci::connectors::{
        traits::{
            Connector, ConnectorType, ExecutionResult,
            KubernetesConfig, LifecycleHook, LifecycleHookType, MongoOperation,
        },
        factory::{BuiltInConnectorFactory, ConnectorFactory},
        ConnectorManager,
        DockerConnector,
        KubernetesConnector,
    };    
    ...
    async fn test_connector_type_display() {
        ...
        assert_eq!(
            ConnectorType::Custom("test".to_string()).to_string(),
            "test"
        );
    }
    ...
    async fn test_connector_manager_determine_connector_type() {
        ...
        assert_eq!(
            manager.determine_connector_type(&docker_step).unwrap(),
            ConnectorType::Docker
        );
        assert_eq!(
            manager.determine_connector_type(&k8s_step).unwrap(),
            ConnectorType::Kubernetes
        );
        assert_eq!(
            manager.determine_connector_type(&shell_step).unwrap(),
            ConnectorType::Docker
        ); // Shell defaults to Docker
    }
    ...
    async fn test_connector_manager_custom_step_validation() {
        ...
        assert_eq!(
            result.unwrap(),
            ConnectorType::Custom("my-plugin".to_string())
        );
    ...
    impl ConnectorFactory for TestConnectorFactory {
        fn create_connector(&self, connector_type: ConnectorType) -> Result<Arc<dyn Connector>> {
            match connector_type {
                ...
                _ => Err(AppError::Unimplemented(format!(
                    "Test factory doesn't support: {}",
                    connector_type
                ))),
            }
        }
    ...
    async fn test_docker_connector_validation_edge_cases() {
        ...
        assert!(connector
            .validate_config(&step_with_empty_dockerfile)
            .is_err());
    ...
    async fn test_kubernetes_config_from_step() {
        ...
        assert_eq!(
            ...
            Some("ci-service-account".to_string())
        );
    ...
    async fn test_connector_manager_integration() {
        ...
        assert_eq!(
            Arc::as_ptr(&docker_connector),
            Arc::as_ptr(&docker_connector2)
        );
    }
    ...
    pub mod mocks {
        ...
        impl MockDockerClient {
            ...
            pub fn execute_command(&self, command: &str) -> Result<(i32, String, String)> {
                ...
                Ok(results
                    .pop()
                    .unwrap_or((0, "default success".to_string(), "".to_string())))
            }
        ...
        impl MockConnector {
            pub fn new(name: String, connector_type: ConnectorType) -> Self {
                Self {
                    name,
                    connector_type,
                    execution_results: Arc::new(Mutex::new(vec![ExecutionResult::success(
        ...
        impl Connector for MockConnector {
            async fn execute_step(
                ...
                Ok(results.pop().unwrap_or_else(|| {
                    ExecutionResult::success("default mock success".to_string())
                }))
            }
    ...
    async fn test_mock_implementations() {
        ...
        let job_name = k8s_client
            .submit_job("apiVersion: batch/v1\nkind: Job")
            .unwrap();
        ...
        let result = mock_connector
            .execute_step(&step, &workspace, &env)
            .await
            .unwrap();
        assert!(!result.is_success());
