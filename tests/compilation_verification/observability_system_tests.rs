...
use RustAutoDevOps::core::networking::valkyrie::observability::adapter_system::{
    AdapterHealth, BuiltInJsonLogger, DependencyHealth, HealthAdapter, HealthStatus, LogLevel,
    LoggingAdapter, MetricsAdapter, NoOpHealthAdapter, NoOpMetricsAdapter, NoOpTracingAdapter,
    ObservabilityConfig, ObservabilityManager, SpanId, SpanStatus, TracingAdapter,
};
...
async fn test_observability_manager_creation_and_basic_usage() {
    ...
    for (name, adapter_health) in health {
        assert!(
            matches!(adapter_health.status, HealthStatus::Healthy),
            "Adapter {} should be healthy",
            name
        );
    }
}
...
async fn test_builtin_json_logger() {
    ...
    logger
        .log(LogLevel::Trace, "Trace message", &[("key", "value")])
        .await;
    logger
        .log(LogLevel::Debug, "Debug message", &[("key", "value")])
        .await;
    logger
        .log(LogLevel::Info, "Info message", &[("key", "value")])
        .await;
    logger
        .log(LogLevel::Warn, "Warn message", &[("key", "value")])
        .await;
    logger
        .log(LogLevel::Error, "Error message", &[("key", "value")])
        .await;
...
async fn test_noop_adapters() {
    ...
    tracing_adapter
        .add_event(span_id, "test_event", &[("key", "value")])
        .await;
    tracing_adapter
        .add_attributes(span_id, &[("attr", "value")])
        .await;
    tracing_adapter
        .set_span_status(span_id, SpanStatus::Ok)
        .await;
    tracing_adapter.end_span(span_id).await;

    health_adapter
        .report_health("test_component", HealthStatus::Healthy)
        .await;
    let deps = health_adapter.check_dependencies().await;
    assert!(deps.is_empty());
    health_adapter
        .record_availability("test_component", true)
        .await;
...
async fn test_concurrent_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            for j in 0..10 {
                manager_clone
                    .record_counter(&format!("concurrent_counter_{}", i), j, &[])
                    .await;
                manager_clone
                    .log(LogLevel::Info, &format!("Concurrent log {} {}", i, j), &[])
                    .await;
            }
        });
        handles.push(handle);
    ...
    for (name, adapter_health) in health {
        assert!(
            matches!(adapter_health.status, HealthStatus::Healthy),
            "Adapter {} should be healthy after concurrent operations",
            name
        );
    }
}
...
async fn test_span_lifecycle() {
    ...
    manager
        .add_event(child_span_id, "processing_started", &[("step", "1")])
        .await;
    manager
        .add_attributes(child_span_id, &[("user_id", "12345")])
        .await;
    ...
    manager
        .set_span_status(parent_span_id, SpanStatus::Ok)
        .await;
