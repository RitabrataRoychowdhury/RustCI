...
use RustAutoDevOps::core::transport::{
    TcpTransport, Transport, TransportConfig, TransportType
};
use RustAutoDevOps::core::secure_transport::{
    AuthenticationManager, EncryptionManager
};
use RustAutoDevOps::core::node_communication_manager::{
    NodeCommunicationManager, CommunicationConfig
};
...
async fn test_high_throughput_message_processing() {
    ...
    println!("Processed {} messages in {:?}", processed_messages, duration);
    println!("Throughput: {:.2} messages/second", messages_per_second);
    
    // Assert minimum performance threshold
    assert!(messages_per_second > 5000.0, "Throughput too low: {:.2} msg/s", messages_per_second);
...
async fn test_concurrent_authentication_load() {
    ...
    assert_eq!(successful_operations, concurrent_operations, "All operations should succeed");
    assert!(operations_per_second > 100.0, "Should handle at least 100 ops/sec");
    assert!(avg_token_time < Duration::from_millis(50), "Token generation should be fast");
    assert!(avg_verify_time < Duration::from_millis(20), "Token verification should be fast");
}
...
async fn test_memory_usage_under_load() {
    ...
    println!("  Generated {} tokens in {:?}", token_count, generation_duration);
    println!("  Verified {} tokens in {:?}", successful_verifications, verification_duration);
    println!("  Total duration: {:?}", total_duration);
    println!("  Memory held {} tokens simultaneously", tokens.len());
    
    // Performance assertions
    assert_eq!(successful_verifications, token_count, "All tokens should verify successfully");
    assert!(generation_duration < Duration::from_secs(30), "Token generation should complete quickly");
    assert!(verification_duration < Duration::from_secs(20), "Token verification should complete quickly");
...
async fn test_message_size_performance() {
    ...
    for (size_name, size_bytes) in sizes {
        ...
        let throughput_serialize = actual_size as f64 / serialize_duration.as_secs_f64() / 1_000_000.0; // MB/s
        let throughput_deserialize = actual_size as f64 / deserialize_duration.as_secs_f64() / 1_000_000.0; // MB/s
        ...
        println!("  Serialize time: {:?} ({:.2} MB/s)", serialize_duration, throughput_serialize);
        println!("  Deserialize time: {:?} ({:.2} MB/s)", deserialize_duration, throughput_deserialize);
        
        // Performance assertions (adjust thresholds as needed)
        match size_name {
            "small" => {
                assert!(serialize_duration < Duration::from_micros(100), "Small message serialization too slow");
                assert!(deserialize_duration < Duration::from_micros(200), "Small message deserialization too slow");
            }
            "medium" => {
                assert!(serialize_duration < Duration::from_millis(5), "Medium message serialization too slow");
                assert!(deserialize_duration < Duration::from_millis(10), "Medium message deserialization too slow");
            }
            "large" => {
                assert!(serialize_duration < Duration::from_millis(50), "Large message serialization too slow");
                assert!(deserialize_duration < Duration::from_millis(100), "Large message deserialization too slow");
            }
            "xlarge" => {
                assert!(serialize_duration < Duration::from_millis(500), "XLarge message serialization too slow");
                assert!(deserialize_duration < Duration::from_millis(1000), "XLarge message deserialization too slow");
            }
            _ => {}
        }
        
        // Throughput should be reasonable
        assert!(throughput_serialize > 1.0, "Serialization throughput too low: {:.2} MB/s", throughput_serialize);
        assert!(throughput_deserialize > 0.5, "Deserialization throughput too low: {:.2} MB/s", throughput_deserialize);
    }
}
...
async fn test_sustained_load_performance() {
    ...
    let producer_handle = tokio::spawn(async move {
        ...
        while start_time.elapsed() < duration {
            ...
            let node_id = Uuid::new_v4();
            let token = producer_auth_manager.generate_token(
                node_id,
            ...
            producer_tx.send((operation_count, operation_duration)).unwrap();
    ...
    // Performance assertions
    assert!(actual_rate > target_rate as f64 * 0.8, "Should achieve at least 80% of target rate");
    assert!(avg_operation_time < Duration::from_millis(10), "Average operation time should be reasonable");
    assert!(p95 < Duration::from_millis(50), "95th percentile should be reasonable");
    assert!(p99 < Duration::from_millis(100), "99th percentile should be reasonable");
}
...
async fn test_connection_scaling() {
    ...
    for connection_count in connection_counts {
        ...
        // Performance assertions
        assert_eq!(total_operations, connection_count * 10, "All operations should succeed");
        assert!(operations_per_second > 100.0, "Should maintain reasonable throughput");
